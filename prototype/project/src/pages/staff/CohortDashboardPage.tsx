...
import { ArrowLeft, TrendingUp, ArrowUp, ArrowDown, Minus, Users, Calendar, Pause, Layers, ClipboardCheck } from 'lucide-react';
...
export default function CohortDashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: cohortRaw }, { data: enrollRaw }] = await Promise.all([
      supabase.from('cohorts').select('id,name,target_count,region,district,program_id,checkin_start_date,start_mode,schedule_paused,baseline_template_id,checkin_template_id').eq('id', cohortId).maybeSingle(),
  ...
  const pagedFarmers = farmers.slice((page - 1) * pageSize, page * pageSize);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-cropguard-slate">{meta.region ?? '—'} · {meta.district}</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 flex-wrap bg-white border border-gray-200 rounded-xl px-4 py-3">
        <div className="flex items-center gap-2 text-xs">
          ...
          <span className="font-semibold text-cropguard-forest capitalize">{meta.start_mode}</span>
          {meta.checkin_start_date && (
            <span className="text-gray-400">· {new Date(meta.checkin_start_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
          )}
        </div>
        <div className="flex items-center gap-2 text-xs">
          <Layers className="w-3.5 h-3.5 text-gray-400" />
          <span className="text-gray-500">Baseline:</span>
          <span className="font-semibold text-cropguard-forest">{meta.baseline_template_id ? 'Linked' : 'None'}</span>
        </div>
        <div className="flex items-center gap-2 text-xs">
          <ClipboardCheck className="w-3.5 h-3.5 text-gray-400" />
          <span className="text-gray-500">Check-in:</span>
          <span className="font-semibold text-cropguard-forest">{meta.checkin_template_id ? 'Linked' : 'None'}</span>
        </div>
        {meta.schedule_paused && (
          <div className="flex items-center gap-1.5 text-xs font-semibold text-red-600">
            <Pause className="w-3.5 h-3.5" /> Schedule Paused
          </div>
        )}
      </div>

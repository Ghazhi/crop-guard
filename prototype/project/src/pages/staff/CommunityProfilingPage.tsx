...
import {
  ...
  TrendingUp, Layers, Activity, ArrowUpRight, ArrowDownRight,
...
export default function CommunityProfilingPage() {
  ...
  const [viewMode, setViewMode] = useState<'card' | 'list'>(() => (localStorage.getItem('community_view_mode') as 'card' | 'list') || 'card');
  const toggleViewMode = () => setViewMode(m => { const n = m === 'card' ? 'list' : 'card'; localStorage.setItem('community_view_mode', n); return n; });
  ...
  const [commLoadAll, setCommLoadAll] = useState(false);
  ...
  const COOP_PAGE_SIZE = 8;
  const [coopLoadAll, setCoopLoadAll] = useState(false);
  ...
  const commPageSize = commLoadAll ? visibleComms.length : COMM_PAGE_SIZE;
  const commTotalPages = Math.max(1, Math.ceil(visibleComms.length / commPageSize));
  const pagedComms = visibleComms.slice((commPage - 1) * commPageSize, commPage * commPageSize);
  const coopPageSize = coopLoadAll ? visibleCoops.length : COOP_PAGE_SIZE;
  const coopTotalPages = Math.max(1, Math.ceil(visibleCoops.length / coopPageSize));
  const pagedCoops = visibleCoops.slice((coopPage - 1) * coopPageSize, coopPage * coopPageSize);
        ...
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search communities…" value={commSearch} onChange={e => { setCommSearch(e.target.value); setCommPage(1); }} className="pl-9" />
            </div>
            <div className="flex items-center gap-2 shrink-0">
              ...
              >
                ...
                <span className="ml-1.5 hidden sm:inline">{viewMode === 'card' ? 'List' : 'Cards'}</span>
              </Button>
              <Button
          ...
          {commLoading ? (
            ...
            <div className="space-y-4">
              ...
              <Pagination page={commPage} totalPages={commTotalPages} onPageChange={setCommPage} totalItems={visibleComms.length} pageSize={commPageSize} onLoadAll={() => { setCommLoadAll(true); setCommPage(1); }} onResetPaging={() => { setCommLoadAll(false); setCommPage(1); }} />
            </div>
          ) : (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 text-xs text-gray-500 uppercase tracking-wider">
                    <tr>
                      <th className="text-left px-4 py-3 font-semibold">Community</th>
                      <th className="text-left px-4 py-3 font-semibold">Region</th>
                      <th className="text-left px-4 py-3 font-semibold">District</th>
                      <th className="text-left px-4 py-3 font-semibold">Leader</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {pagedComms.map(c => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-gray-500">{REGION_LABELS[c.region_code as RegionCode] ?? c.region_code}</td>
                        <td className="px-4 py-3 text-gray-500">{c.district}</td>
                        <td className="px-4 py-3 text-gray-500">{c.leader_name ?? '—'}</td>
                        <td className="px-4 py-3">
                          {c.socioeconomic_status ? (
                            <span className="text-[10px] font-semibold bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full capitalize">{c.socioeconomic_status.replace('_', '-')}</span>
                          ) : '—'}
                        </td>
      ...
      {tab === 'cooperative' && (
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
              <Input placeholder="Search cooperatives…" value={coopSearch} onChange={e => { setCoopSearch(e.target.value); setCoopPage(1); }} className="pl-9" />
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <Button
                variant="outline"
          ...
          {coopLoading ? (
            ...
            <div className="space-y-4">
              ...
              <Pagination page={coopPage} totalPages={coopTotalPages} onPageChange={setCoopPage} totalItems={visibleCoops.length} pageSize={coopPageSize} onLoadAll={() => { setCoopLoadAll(true); setCoopPage(1); }} onResetPaging={() => { setCoopLoadAll(false); setCoopPage(1); }} />
            </div>
          ) : (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 text-xs text-gray-500 uppercase tracking-wider">
                    <tr>
                      <th className="text-left px-4 py-3 font-semibold">Cooperative</th>
                      <th className="text-left px-4 py-3 font-semibold">Community</th>
                      <th className="text-left px-4 py-3 font-semibold">Members</th>
                      <th className="text-left px-4 py-3 font-semibold">Crops</th>
                      <th className="text-left px-4 py-3 font-semibold">Chairman</th>
                      <th className="text-right px-4 py-3 font-semibold"></th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {pagedCoops.map(coop => (
                      <tr
                        ...
                        onClick={() => setDetailCoop(coop)}
                      >
                        <td className="px-4 py-3 font-medium text-gray-900">{coop.name}</td>
                        <td className="px-4 py-3 text-gray-500">{coop.community_name ?? '—'}</td>
                        <td className="px-4 py-3 text-gray-500">{coop.member_count}</td>
                        <td className="px-4 py-3">
                          <div className="flex flex-wrap gap-1">
                            {(coop.primary_crops ?? []).slice(0, 3).map(c => (
                              <span key={c} className="text-[10px] bg-green-50 text-green-700 px-1.5 py-0.5 rounded font-medium capitalize">
                                {CROP_OPTIONS.find(o => o.value === c)?.label ?? c}
                              </span>
                            ))}
                        ...
                        <td className="px-4 py-3 text-gray-500">{coop.chairman_name ?? '—'}</td>
                        <td className="px-4 py-3 text-right">
                          <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-700"

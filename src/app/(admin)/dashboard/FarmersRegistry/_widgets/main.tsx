...
import { FARMER_REGISTRATION_FORM_ID, FARMER_ASSIGN_AGENT_FORM_ID, FARMER_ENROLL_FORM_ID } from '@/dataCenter/formEngine'
...
export const ZONE_COLORS: Record<FriZone, string> = {
...
export const ZONE_RISK: Record<FriZone, string> = {
...
export function enrolmentWorkflowDetail(farmerId: string, enr: FarmerEnrollment) {
...
export function communityDetail(f: Farmer) {
...
function AddFarmerSheet({ open, onClose, onSave, programs }: {
  ...
  return (
    <SheetTemplate
      open={open}
      onClose={onClose}
      title={currentStep?.name ?? 'Register Farmer'}
...
/** Maps the dynamic answers bag onto the typed payload the registry list expects. */
function toAddFarmerForm(form: StepperForm): AddFarmerForm {
  return {
...
function AssignAgentSheet({ open, onClose, farmer, farmerCount }: {
  ...
  const agentOptions = useDynamicFieldOptions({ agents: MOCK_AGENTS.map(a => ({ value: a, label: a })) })
  ...
  return (
    ...
    >
      ...
      <DynamicFormRenderer
        ...
        stepId={step.id}
        values={{ agent }}
        onChange={(_k, v) => setAgent(String(v ?? ''))}
...
function EnrollSheet({ open, onClose, farmerCount, programs }: {
  ...
  // Field list, order, labels and required-ness all come from Configuration > Forms.
  const enrollConfig = useFormConfig(FARMER_ENROLL_FORM_ID)
  const enrollStep = enrollConfig.steps[0]
  const enrollOptions = useDynamicFieldOptions({ programs, selectedProgramId: programId })
  ...
  return (
    ...
    >
      ...
      <DynamicFormRenderer
        form={enrollConfig.form}
        stepId={enrollStep.id}
        values={{ programId, cohortId }}
        onChange={(k, v) => {
          const next = String(v ?? '')
          // switching program invalidates whichever cohort was picked under the old one
          if (k === 'programId') { setProgramId(next); setCohortId('') } else { setCohortId(next) }
        }}
        optionsOverride={enrollOptions}
        labelVariant="compact"
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        ...
        <YesNoTemplate value={form.ownsTractor} onChange={v => setField('ownsTractor', v)} />
      </div>
      <div className="grid grid-cols-2 gap-3">
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <YesNoTemplate value={form.ownsHouse} onChange={v => setField('ownsHouse', v)} />
        </div>
      ...
      <div>
        ...
        <YesNoTemplate value={form.otherBusiness} onChange={v => setField('otherBusiness', v)} />
      </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        ...
        <YesNoTemplate value={form.hasAgricInsurance} onChange={v => setField('hasAgricInsurance', v)} />
      </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <ChipSelectTemplate options={EDIT_ASSETS.map(a => ({ value: a, label: a }))} value={form.desiredAssets} onChange={v => setField('desiredAssets', v)} />
      </div>
      <div>
        ...
        <YesNoTemplate value={form.inputCredit} onChange={v => setField('inputCredit', v)} />
      </div>
    </div>
  )
}
...
export function Main() {
  ...
  // Filters
  const [search,         setSearch]         = usePersistedState('fr-search', '')
  const [filterProgram,  setFilterProgram]  = usePersistedState('fr-program', '')
  const [filterCohort,   setFilterCohort]   = usePersistedState('fr-cohort', '')
  const [filterEnrolled, setFilterEnrolled] = usePersistedState('fr-enrolled', '')
  const [filterZone,     setFilterZone]     = usePersistedState('fr-zone', '')
  ...
  return (
    <div className="p-6 space-y-5" style={{ background: 'var(--brand-gray)', minHeight: '100vh' }}>
      ...
      {/* ── Columns ────────────────────────────────────────────────────────────── */}
      {!loading && (
        <div className="flex items-center justify-end gap-2 flex-wrap px-1">

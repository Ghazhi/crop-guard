...
import { FileSpreadsheet, Layers, Calendar, BarChart3, Users, PanelLeftClose, PanelLeftOpen } from 'lucide-react'
import { usePersistedState } from '@/lib/usePersistedState'
import { ScrollTabsTemplate } from '@/customComponents/ScrollTabsTemplate'
import { SurveyTemplateSection } from './SurveyTemplateSection'
import { SurveyScheduleSection } from './SurveyScheduleSection'
import { SurveyMetricsSection } from './SurveyMetricsSection'
import { SurveyOverrideSection } from './SurveyOverrideSection'
...
const SECTIONS: { key: SectionKey; icon: React.ElementType; label: string; desc: string }[] = [
  { key: 'templates', icon: Layers,         label: 'Survey Template',  desc: 'Create reusable survey templates with questions'    },
  { key: 'schedules', icon: Calendar,       label: 'Survey Schedule',  desc: 'Schedule a survey template against a cohort'        },
  { key: 'metrics',   icon: BarChart3,      label: 'Survey Metrics',   desc: 'Summary of survey template and schedule activity'   },
  { key: 'overrides', icon: Users,          label: 'Survey Override',  desc: 'Pause surveys for specific farmers within a cohort' },
]

export function SurveyConfigShell() {
  const [section, setSection] = usePersistedState<SectionKey>('surveyConfigV2.section', 'templates')
  const [collapsed, setCollapsed] = usePersistedState('surveyConfigV2.navCollapsed', false)
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-3">
        <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: 'var(--brand-mint)' }}>
          <FileSpreadsheet className="w-4.5 h-4.5" style={{ color: 'var(--brand-forest)' }} />
        </div>
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <h2 className="text-base font-bold text-gray-900">Survey Configuration</h2>
            <button
          ...
          <p className="text-xs text-gray-500">Configure survey templates, schedules, metrics, and farmer overrides</p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-5 items-start">
        ...
        <div className="w-full flex-1 min-w-0">
          {section === 'templates' && <SurveyTemplateSection />}
          {section === 'schedules' && <SurveyScheduleSection />}
          {section === 'metrics' && <SurveyMetricsSection />}
          {section === 'overrides' && <SurveyOverrideSection />}
        </div>
      </div>
    </div>
  )

...
import {
  ClipboardCheck,
  ChevronRight, TrendingUp, Leaf, ChevronDown,
} from 'lucide-react';
...
function zoneOf(score: number | null): ZoneInfo {
  if (score === null) return { name: 'none',    label: 'Not Assessed', color: '#9CA3AF', status: 'none'        };
  if (score >= 80)    return { name: 'leader',  label: 'Leader',       color: '#1A3D2B', status: 'provisional' };
  if (score >= 60)    return { name: 'builder', label: 'Builder',      color: '#3D7A56', status: 'provisional' };
  if (score >= 40)    return { name: 'learner', label: 'Learner',      color: '#E8963A', status: 'provisional' };
  return                     { name: 'starter', label: 'Starter',      color: '#D94F3D', status: 'provisional' };
}
  ...
  useEffect(() => {
    ...
    supabase.from('farmers').select('id').eq('user_id', profile.id).maybeSingle()
      .then(({ data: f }) => {
        ...
        // Live opportunities from enrolled program
        supabase.from('enrollments').select('program_id').eq('farmer_id', f.id).eq('status', 'active').maybeSingle()
          .then(({ data: enr }) => {
            if (enr?.program_id) {
              (supabase.from('interventions_catalog') as any)
                .select('id, name, type, min_fri')
                .eq('status', 'Active')
                .eq('program_id', enr.program_id)
                .order('min_fri')
                .then(({ data: ivRows }) => {
                  if (ivRows) setOpps(ivRows as OppSummary[]);
                });
            }
          });
      });
    ...
    // Programme name
    supabase.from('enrollments').select('program_id').eq('farmer_id', profile.id).eq('status', 'active').maybeSingle()
      .then(({ data: enr }) => {
        if (enr?.program_id) {
          supabase.from('programs').select('name').eq('id', enr.program_id).maybeSingle()
            .then(({ data: prog }) => {
              if (prog) setProgram((prog as any).name);
            });
        }
      });
  }, [profile]);
        ...
        <div>
          ...
          <div className="flex gap-2.5 overflow-x-auto pb-1 -mx-4 px-4 snap-x">
            {opps.slice(0, 6).map(opp => {
              const eligible = currentScore !== null && currentScore >= opp.min_fri;
              return (
                <Link key={opp.id} to="/farmer/opps"
                  className={cn(
                  ...
                  <p className="text-xs font-semibold text-cropguard-forest truncate">{opp.name}</p>
                  <p className="text-[10px] text-cropguard-slate mt-0.5 leading-tight">{opp.type}</p>
                  <Badge className={cn('mt-1.5 text-[9px] border-0',
                    eligible ? 'bg-cropguard-mint text-cropguard-dark' : 'bg-gray-100 text-gray-500')}>
                    {eligible ? 'Eligible' : `Need ${opp.min_fri} FRI`}
                  </Badge>
                </Link>
              );
            })}

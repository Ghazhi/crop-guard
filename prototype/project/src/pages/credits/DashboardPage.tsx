...
export default function CreditsDashboard() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const [friRes, enrollRes, baselineRes] = await Promise.all([
        ...
        // Filter FRI scores to enrolled farmers only
        const enrolledFri = latest.filter(r => enrolledIds.has(r.farmer_id));

        setStats({
          totalFarmers:    enrolledIds.size,
          enrolled:        enrolledIds.size,
          scored:          enrolledFri.length,
          highRisk:        enrolledFri.filter(r => r.zone === 'high' || r.zone === 'critical').length,
          lowRisk:         enrolledFri.filter(r => r.zone === 'low').length,
          pendingBaseline: enrolledIds.size - [...enrolledIds].filter(id => basedIds.has(id)).length,
        });

        // Fetch farmer names for the 10 most recent scored (enrolled only)
        const top10 = enrolledFri.slice(0, 10);

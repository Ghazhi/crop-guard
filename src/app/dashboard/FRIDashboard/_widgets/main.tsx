...
function FarmerDetailSheet({ farmer, open, onClose }: {
  ...
  useEffect(() => { if (open) { setTab('overview'); setShowAllScores(false); setExpandedWeek(null) } }, [open])
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                  {[
                    ...
                    { Icon: CalendarDays, label: 'Age',       value: farmer.dateOfBirth ? `${Math.floor((Date.now() - new Date(farmer.dateOfBirth).getTime()) / (365.25 * 24 * 60 * 60 * 1000))} yrs` : '—' },

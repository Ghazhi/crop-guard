...
// ── Cocoa-specific weekly activities ──────────────────────────────────────────
const COCOA_P1_ACTIVITIES = [
  "cocoa_shade_management", "cocoa_pruning", "cocoa_fertiliser",
  "cocoa_pest_scouting", "cocoa_disease_mgmt",
];
const COCOA_P2_ACTIVITIES = [
  "cocoa_mulching", "cocoa_composting", "cocoa_erosion_control",
  "cocoa_water_conservation", "cocoa_no_burning",
];
const COCOA_P3_ACTIVITIES = [
  "cocoa_attends_training", "cocoa_follows_agronomist",
  "cocoa_cooperative_visits", "cocoa_cooperative_member",
];
const COCOA_P4_ACTIVITIES = [
  "cocoa_repayment_history", "cocoa_savings_habit",
  "cocoa_harvest_quality", "cocoa_lbc_confirmed",
];

// ── Cocoa-specific ECI items ──────────────────────────────────────────────────
const COCOA_ECI_ITEMS = [
  "cocoa_income_debt", "cocoa_financial_stability", "cocoa_cocobod_registration",
  "cocoa_traceability_polygon", "cocoa_sustainability_cert",
];

function getActivitySets(cropType?: string) {
  if (cropType === "cocoa") {
    return {
...
function calcECIRaw(eciResponses: Record<string, number>, eciItems: string[] = ECI_ITEMS): number {
  return eciItems.reduce((s, k) => {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // §5.1 Compute pillar scores (crop-aware)
    const acts = getActivitySets(crop_type);
    const p1 = Math.round(calcPillarScore(acts.p1, farmer_responses, agent_verifications, PILLAR_WEIGHTS.P1));
    const p2 = Math.round(calcPillarScore(acts.p2, farmer_responses, agent_verifications, PILLAR_WEIGHTS.P2));
    const p3 = Math.round(calcPillarScore(acts.p3, farmer_responses, agent_verifications, PILLAR_WEIGHTS.P3));
    const p4 = Math.round(calcPillarScore(acts.p4, farmer_responses, agent_verifications, PILLAR_WEIGHTS.P4));
    const total = p1 + p2 + p3 + p4;

    // §5.5 Provisional if any verified but not all
    const allKeys = [...acts.p1, ...acts.p2, ...acts.p3, ...acts.p4];
    ...
    // §5.8 ECI (crop-aware)
    const eciRaw = calcECIRaw(eci_responses ?? {}, acts.eci);

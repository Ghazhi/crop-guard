...
CREATE POLICY "Farmers view their own enrollments"
  ...
  USING (
    EXISTS (
      ...
      WHERE farmers.id = enrollments.farmer_id
        AND farmers.user_id = auth.uid()
    )
  );

CREATE POLICY "Agents view enrollments they manage"
  ON enrollments FOR SELECT
  TO authenticated
...
CREATE POLICY "Farmers view their own checkins"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM farmers
      WHERE farmers.id = weekly_checkins.farmer_id
        AND farmers.user_id = auth.uid()
    )
  );
...
CREATE POLICY "Staff and admin view checkins in their org"
  ...
  USING (
    ...
    AND EXISTS (
      SELECT 1 FROM enrollments
      JOIN programs ON programs.id = enrollments.program_id
      WHERE enrollments.id = weekly_checkins.enrollment_id
        AND programs.organisation_id = get_my_org_id()
    )
  );
...
CREATE POLICY "Agents view activities on their checkins"
  ON checkin_activities FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM weekly_checkins
      WHERE weekly_checkins.id = checkin_activities.checkin_id
        AND weekly_checkins.agent_id = auth.uid()
    )
  );
...
CREATE POLICY "Staff and admin view activities in their org"
  ...
  USING (
    ...
    AND EXISTS (
      SELECT 1 FROM weekly_checkins
      JOIN enrollments ON enrollments.id = weekly_checkins.enrollment_id
      JOIN programs ON programs.id = enrollments.program_id
      WHERE weekly_checkins.id = checkin_activities.checkin_id
        AND programs.organisation_id = get_my_org_id()
    )
...
CREATE POLICY "Farmers view activities for their own checkins"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM weekly_checkins
      JOIN farmers ON farmers.id = weekly_checkins.farmer_id
      WHERE weekly_checkins.id = checkin_activities.checkin_id
        AND farmers.user_id = auth.uid()
    )
  );
...
CREATE POLICY "Agents can add checkin activities"
  ON checkin_activities FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM weekly_checkins
      WHERE weekly_checkins.id = checkin_id
        AND weekly_checkins.agent_id = auth.uid()
    )
  );
...
CREATE POLICY "Farmers view their own verifications"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM farmers
      WHERE farmers.id = verifications.farmer_id
        AND farmers.user_id = auth.uid()
    )
  );
...
CREATE POLICY "Agents view verif activities for their verifications"
  ON verification_activities FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM verifications
      WHERE verifications.id = verification_activities.verification_id
        AND verifications.agent_id = auth.uid()
    )
  );
...
CREATE POLICY "Staff view verif activities in their org"
  ...
  USING (
    ...
    AND EXISTS (
      SELECT 1 FROM verifications
      JOIN farmers ON farmers.id = verifications.farmer_id
      WHERE verifications.id = verification_activities.verification_id
        AND farmers.organisation_id = get_my_org_id()
    )
  );
...
CREATE POLICY "Agents can insert verif activities"
  ON verification_activities FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM verifications

...
export default function EnrollmentWorkflowPage() {
  ...
  const handleAction = async () => {
    ...
    if (actionType === 'decline') {
      ...
      // Auto-create auth account when enrollment becomes active
      try {
        ...
        if (accessToken) {
          const { data: farmerRow } = await supabase
            .from('farmers')
            .select('phone, full_name, organisation_id')
            .eq('id', activeEnr.farmer_id)
            .maybeSingle();
          if (farmerRow?.phone) {
            await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/create-farmer-account`, {
              ...
              body: JSON.stringify({
                ...
                full_name: farmerRow.full_name,
                organisation_id: farmerRow.organisation_id,
              }),
            });
          }
        }

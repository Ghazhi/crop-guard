...
export default function FarmerDetailPage() {
  ...
  const [flagLoadAll,   setFlagLoadAll]   = useState(false);
  ...
  const flagTotalPages = Math.max(1, Math.ceil(riskFlags.length / (flagLoadAll ? riskFlags.length : FLAG_PAGE_SIZE)));
  const pagedFlags = riskFlags.slice((flagPage - 1) * (flagLoadAll ? riskFlags.length : FLAG_PAGE_SIZE), flagPage * (flagLoadAll ? riskFlags.length : FLAG_PAGE_SIZE));
    ...
    <div className="p-3 sm:p-6 max-w-5xl mx-auto space-y-4 sm:space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
        <div className="flex items-start gap-3 sm:gap-4 flex-wrap">
          {/* Avatar / photo */}
          <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl bg-emerald-100 flex items-center justify-center shrink-0 text-xl sm:text-2xl font-bold text-emerald-700">
            {farmer.full_name.charAt(0).toUpperCase()}
          </div>
          <div className="flex-1 min-w-0">
            <h1 className="text-lg sm:text-xl font-bold text-gray-900">{farmer.full_name}</h1>
            <p className="text-xs sm:text-sm text-gray-500 mt-0.5">{farmer.asinyo_id} · {farmer.community}, {farmer.district}</p>
      ...
      <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
        ...
        {([
          ...
          <button
            ...
            className={cn(
              'px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors',
              viewMode === key
      ...
      {riskFlags.length > 0 && (
        <Card>
          ...
          <CardContent className="p-0">
            ...
            <Pagination page={flagPage} totalPages={flagTotalPages} onPageChange={setFlagPage} totalItems={riskFlags.length} pageSize={flagLoadAll ? riskFlags.length : FLAG_PAGE_SIZE} onLoadAll={() => { setFlagLoadAll(true); setFlagPage(1); }} onResetPaging={() => { setFlagLoadAll(false); setFlagPage(1); }} />
          </CardContent>
        </Card>
      )}
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className="w-full flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-3 text-left hover:bg-gray-50/50 transition-colors"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {int.value_description && <p className="text-xs text-gray-500 truncate sm:whitespace-normal">{int.value_description}</p>}
                  </div>

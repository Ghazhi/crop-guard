...
export default function PortfolioOverviewPage() {
  ...
  useEffect(() => { setFlagPage(1); }, [progId, cohortId]);

  const flagPageSize = flagLoadAll ? flags.length : FLAG_PAGE_SIZE;
  const flagTotalPages = Math.max(1, Math.ceil(flags.length / flagPageSize));
            ...
            : flags.length === 0
              ...
              : <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {flags.slice((flagPage - 1) * flagPageSize, flagPage * flagPageSize).map(f => (
                        <tr
                          key={f.id}
                          className={cn(
                  ...
                  <Pagination page={flagPage} totalPages={flagTotalPages} onPageChange={setFlagPage} totalItems={flags.length} pageSize={flagPageSize} onLoadAll={() => { setFlagLoadAll(true); setFlagPage(1); }} onResetPaging={() => { setFlagLoadAll(false); setFlagPage(1); }} />
                </div>
          }
        </CardContent>
      </Card>

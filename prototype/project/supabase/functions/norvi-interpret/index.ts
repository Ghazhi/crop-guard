...
function buildPrompt(type: OutputType, ctx: ScoreContext): string {
  ...
  const verificationNote = ctx.is_agent_only
    ? "\nIMPORTANT: This score was recorded via agent-only verification — the field agent assessed the farmer directly without a farmer self-check-in. All farmer responses were agent-observed, not self-reported. ECI and credit score data may be absent for this record."
    : "";
  const eciNote = !ctx.has_eci
    ? "\nNote: ECI (Economic Commitment Index) data was not collected for this assessment. Credit score and financial capacity assessments should be treated as incomplete."
    : "";
...
3. Credit recommendation — state whether credit is recommended, any conditions or mitigants, and a suggested next review point${ctx.is_agent_only ? "\n\nIMPORTANT CONTEXT: This assessment was conducted via agent-only verification (the field agent assessed the farmer directly, without a farmer self-check-in). Scores reflect agent observations, not self-reported data." : ""}${!ctx.has_eci ? "\n\nIMPORTANT CONTEXT: ECI (Economic Commitment Index) data was not collected. The credit score is not available. The credit recommendation must explicitly state that financial capacity assessment is incomplete and a follow-up ECI assessment is required before extending credit." : ""}
...
Pillar scores — Farm Management: ${ctx.p1_score}/30, Climate Resilience: ${ctx.p2_score}/30, Economic Resilience: ${ctx.p3_score}/20, Welfare: ${ctx.p4_score}/20${flags}${verificationNote}${eciNote}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { farmer_id, week_number, fri_score_id, output_type, custom_prompt } = body;
    ...
    // Rate limit: check for existing output in last 7 days (skip cache for custom prompts)
    if (!custom_prompt) {
    ...
    const [{ data: farmerRaw }, { data: scoreRaw }, enrollRaw] = await Promise.all([
      ...
      supabase.from("farmer_fri_scores")
        .select("total_score,p1_score,p2_score,p3_score,p4_score,eci_score,credit_score,season_average,zone,is_provisional,raw_responses")
    ...
    const rawResponses = score.raw_responses as any;
    const isAgentOnly = rawResponses?.verification_mode === "agent_only";
    const hasEci = rawResponses?.eci != null || Number(score.eci_score) > 0;

    const scoreContext = `Farmer: ${farmer.full_name}
Week ${week_number} FRI Score: ${score.total_score}/100 (${score.zone}, ${score.is_provisional ? "provisional" : "verified"})
Credit Score: ${score.credit_score !== null ? score.credit_score : "Not computed"}
Pillar scores — P1 Farm Mgmt: ${score.p1_score}/30 | P2 Climate: ${score.p2_score}/30 | P3 Economic: ${score.p3_score}/20 | P4 Welfare: ${score.p4_score}/20${hasEci ? ` | ECI: ${score.eci_score}` : " | ECI: Not collected"}${score.season_average !== null ? `\nSeason average: ${score.season_average}/100` : ""}${riskFlags.length > 0 ? `\nActive risk flags: ${riskFlags.join("; ")}` : ""}${isAgentOnly ? "\nAssessment method: Agent-only verification (agent observed directly, no farmer self-check-in)" : ""}${!hasEci ? "\nNote: ECI data not collected — financial capacity assessment is incomplete" : ""}`;

    const prompt = custom_prompt
      ? `${custom_prompt}\n\n--- Farmer Data ---\n${scoreContext}`
      : buildPrompt(output_type, {
    ...
    const maxTokens = custom_prompt
      ? Math.max(MAX_TOKENS[output_type], 1000)
      : MAX_TOKENS[output_type];

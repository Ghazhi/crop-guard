...
export function Main() {
  ...
  const [summary,  setSummary]  = useState<FRISummary | null>(null)
  const [programs, setPrograms] = useState<ProgramOption[]>([])
  const [loading,  setLoading]  = useState(true)

  const [search,        setSearch]        = usePersistedState('fri-search', '')
  const [filterProgram, setFilterProgram] = usePersistedState('fri-program', '')
  const [filterCohort,  setFilterCohort]  = usePersistedState('fri-cohort', '')
  const [filterStatus,  setFilterStatus]  = usePersistedState('fri-status', '')
  const [filterZone,    setFilterZone]    = usePersistedState<FriZone | ''>('fri-zone', '')
  const [showCards,     setShowCards]     = usePersistedState('fri-show-cards', false)

  const [focusFarmer, setFocusFarmer] = useState<FRIFarmer | null>(null)
  const [detailOpen,  setDetailOpen]  = useState(false)

  useEffect(() => {
    Promise.all([getFRIFarmers(), getFRISummary(), getFRIPrograms()]).then(([f, s, p]) => {
      setFarmers(f); setSummary(s); setPrograms(p); setLoading(false)
    })
  }, [])

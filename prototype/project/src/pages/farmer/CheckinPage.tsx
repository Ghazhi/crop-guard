...
import {
  ...
  Clock, ShieldCheck, CalendarDays, Lock, XCircle,
} from 'lucide-react';
...
function AgentVerdictBadge({ verdict }: { verdict: string | null }) {
  if (!verdict) {
    return (
  ...
  return (
    <span className="inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-full bg-gray-100 text-gray-600 capitalize">
      <Clock className="w-3 h-3" />
      {verdict.replace(/_/g, ' ')}
    </span>
  );
...
export default function CheckinPage() {
  ...
  const [acts,          setActs]          = useState<ActivityState[]>(() =>
    WEEKLY_ACTIVITIES.map(a => ({ id: a.id, answer: null, photo: null, flagged: false, agentVerdict: null }))
  );
  ...
  const checkinTemplateIdRef = useRef<string | null>(null);
  ...
  const loadState = useCallback(async () => {
    ...
    let cohortCheckinTemplateId: string | null = null;
    let cohortSchedulePaused = false;

    if (cohortId) {
      const { data: cohort } = await supabase
        .from('cohorts')
        .select('checkin_start_date, checkin_window_days, checkin_grace_days, start_mode, checkin_template_id, schedule_paused')
        .eq('id', cohortId)
        .maybeSingle();
      if (cohort) {
        config = {
        ...
        cohortCheckinTemplateId = (cohort as any).checkin_template_id ?? null;
        checkinTemplateIdRef.current = cohortCheckinTemplateId;
        cohortSchedulePaused = (cohort as any).schedule_paused ?? false;
      }
    }

    // Check per-farmer override
    if (cohortId && !cohortSchedulePaused) {
      const { data: override } = await (supabase.from('cohort_farmer_overrides') as any)
        .select('is_paused')
        .eq('cohort_id', cohortId)
        .eq('farmer_id', f.id)
        .maybeSingle();
      if (override?.is_paused) cohortSchedulePaused = true;
    }

    setCohortConfig(config);
    configRef.current = config;

    if (cohortSchedulePaused) {
      setPageStatus('window_closed');
      return;
    }

    // 3. Load activities — try cohort-linked checkin template first, then crop-specific checkin_questions, then fall back
    ...
    // 3a. Try cohort-linked checkin template
    if (cohortCheckinTemplateId && win0.hasStarted) {
      const { data: tplItems } = await supabase
        .from('checkin_template_items')
        .select('id, component, activity_code, label, description, sort_order, week_number')
        .eq('checkin_template_id', cohortCheckinTemplateId)
        .eq('is_active', true)
    ...
    // 3b. Fall back to crop-specific checkin_questions
    if (!activeActivities.length && farmerCrop && win0.hasStarted && profile.organisation_id) {
    ...
    // 3c. Fall back to weekly_activity_config, then hardcoded defaults
    ...
    setActs(activeActivities.map(a => ({ id: a.id, answer: null, photo: null, flagged: false, agentVerdict: null })));
    ...
    const { data: responses } = await (supabase.from('farmer_checkin_responses') as any)
      .select('activity_code, farmer_response, is_flagged, photo_url, agent_response')
      .eq('checkin_id', ci.id);
    ...
    if (responses?.length) {
      setActs(prev => prev.map(a => {
        ...
        return {
          ...
          photo:        r.photo_url,
          agentVerdict: r.agent_response ?? null,
        };
      }));
    }
    ...
    if (!cid) {
      const { data: ci, error } = await (supabase.from('farmer_checkins') as any)
        .insert({
          ...
          verification_mode: 'farmer_then_agent',
          checkin_template_id: checkinTemplateIdRef.current,
        })
                      ...
                      return (
                        <div key={act.id} className="py-3">
                          ...
                          <div className="mt-2 flex items-center gap-2 flex-wrap">
                            ...
                            <AgentVerdictBadge verdict={st?.agentVerdict ?? null} />
                          </div>
                        </div>
                      );
                    })}

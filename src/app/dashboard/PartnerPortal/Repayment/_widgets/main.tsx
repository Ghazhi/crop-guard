...
import type { Farmer, FriZone } from '@/app/dashboard/FarmersRegistry/_logics/interface'
...
export function Main() {
  ...
  const partnerCohortIds = new Set([
    ...interventions.flatMap(iv =>
      iv.partnerAssignments?.find(pa => pa.partnerId === partnerId)?.cohorts.map(c => c.cohortId) ?? []
    ),
    ...programs.flatMap(p => p.cohorts.map(c => c.id)),
  ])

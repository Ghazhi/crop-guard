...
import {
  ...
  PanelLeftClose, PanelLeftOpen, Coins, Users, Zap, Wallet, Award,
} from 'lucide-react';
...
const NAV_ITEMS = [
  ...
  { to: '/credits/applications',   icon: FileText,        label: 'Opportunities'         },
  { to: '/credits/enrollments',    icon: Wallet,          label: 'Enrollments'           },
  { to: '/credits/offtake',        icon: FileText,        label: 'Offtake Agreements'    },
  { to: '/credits/disbursements',  icon: Wallet,          label: 'Disbursement Ledger'   },
  { to: '/credits/farmers',        icon: Users,           label: 'Farmer Profiles'       },
  { to: '/credits/scoring',        icon: TrendingUp,      label: 'Risk Scoring'          },
  { to: '/credits/scores',        icon: Award,           label: 'Farmer Scores'        },
  { to: '/credits/interventions',  icon: Zap,             label: 'Opportunity Pathways'  },
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className={cn(
        ...
        <div className="border-t border-white/10 p-3">
          {!collapsed && (
            <button
              onClick={() => navigate('/credits/configuration?tab=profile')}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-white text-sm font-medium truncate group-hover:text-blue-200 transition-colors">{profile?.full_name || 'Credits Officer'}</p>
      ...
      <div className={cn('flex-1 flex flex-col min-h-screen transition-all duration-200', mainML)}>
        <header className="sticky top-0 z-10 bg-white border-b border-gray-200 h-14 flex items-center px-4 gap-3">
          ...
          >
            ...
            <span className="text-sm text-gray-700 hidden sm:block group-hover:text-blue-700 transition-colors">{profile?.full_name}</span>
          </button>
        </header>

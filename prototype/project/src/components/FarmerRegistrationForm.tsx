...
import { CROP_OPTIONS, REGION_NAME_OPTIONS, DISTRICTS_BY_REGION, REGION_CODE_BY_NAME } from '@/lib/constants';
...
export interface FarmerFormData {
  ...
  gps_address:                    string;
  // Personal background (paper form additions)
  residential_address:            string;
  has_disability:                 string;
  disability_specify:             string;
  is_refugee_displaced:           string;
  refugee_specify:                string;
  // Occupation & Farming
  primary_occupation:             string;
  secondary_occupation:           string;
  farming_type:                   string;
  farm_location:                  string;
  main_buyer_market:              string;
  // Financial
  loan_type:                      string;
  loan_tenor:                     string;
  ...
  existing_loan_detail:           string;
  farm_loss_history:              string;
  has_agric_insurance:            string;
  willing_sell_via_asinyo:        string;
  willing_repay:                  string;
...
interface Community { id: string; name: string; region_code: string | null; }
interface Cooperative { id: string; name: string; community_id: string | null; }
interface Program { id: string; name: string; }
interface Cohort { id: string; name: string; program_id: string; }

interface FarmerRegistrationFormProps {
  onComplete: (data: FarmerFormData, andExit?: boolean) => Promise<void>;
  onBack?: () => void;
  saving?: boolean;
  saveError?: string;
  initialStep?: number;
...
const STEPS = ['Personal', 'Identity', 'Farm', 'Household', 'Financial', 'Support'];
...
const FARMING_TYPE_OPTIONS = [
  { value: 'own_land',     label: 'On my own land' },
  { value: 'leased_land',  label: 'On leased land' },
  { value: 'shared',       label: 'Shared / communal' },
];

const LOAN_TYPE_OPTIONS = [
  { value: 'none',          label: 'No loan needed' },
  { value: 'input_credit',  label: 'Input Credit' },
  { value: 'cash_loan',     label: 'Cash Loan' },
  { value: 'asset_loan',     label: 'Asset Loan' },
];

const LOAN_TENOR_OPTIONS = [
...
const REPAYMENT_OPTIONS = [
  { value: 'harvest_lumpsum',  label: 'Lump sum at harvest' },
  { value: 'installments',     label: 'Monthly installments' },
  { value: 'after_sale',       label: 'After produce sale' },
];
...
const SUPPORT_NEEDED_OPTIONS = [
  'Seeds', 'Fertilizer', 'Credit/Input Loan', 'Tractor/Ploughing',
  'Extension/Agronomy', 'Agrochemicals', 'Market Access', 'Storage',
];
        ...
        <div key={i} className="flex items-center gap-1">
          <button
            ...
            className={cn(
              ...
              onStepClick ? 'cursor-pointer hover:ring-2 hover:ring-cropguard-mid' : 'cursor-default',
...
export default function FarmerRegistrationForm({
  ...
  const [s1, setS1] = useState<S1 | null>(mode === 'edit' ? {
    ...
    region:        (initialData as any).region ?? (initialData as any).region_code ?? '',
  ...
  const [personalData, setPersonalData] = useState({
    ...
    disability_specify:    initialData.disability_specify ?? '',
    is_refugee_displaced:  initialData.is_refugee_displaced ?? '',
    refugee_specify:       initialData.refugee_specify ?? '',
  });

  const [occupationData, setOccupationData] = useState({
    primary_occupation:   initialData.primary_occupation ?? '',
    secondary_occupation: initialData.secondary_occupation ?? '',
    farming_type:         initialData.farming_type ?? '',
    farm_location:        initialData.farm_location ?? '',
    main_buyer_market:    initialData.main_buyer_market ?? '',
  });

  const [financialData, setFinancialData] = useState({
    loan_type:             initialData.loan_type ?? '',
    loan_tenor:            initialData.loan_tenor ?? '',
    preferred_repayment:   initialData.preferred_repayment ?? '',
    average_income:        initialData.average_income ?? '',
    account_type:          initialData.account_type ?? '',
    account_number:        initialData.account_number ?? '',
    bank_name:             initialData.bank_name ?? '',
    mobile_money_network:  initialData.mobile_money_network ?? '',
    other_income_sources:  initialData.other_income_sources ?? '',
    existing_loans:        initialData.existing_loans ?? '',
    existing_loan_detail:  initialData.existing_loan_detail ?? '',
    farm_loss_history:     initialData.farm_loss_history ?? '',
    has_agric_insurance:   initialData.has_agric_insurance ?? '',
    willing_sell_via_asinyo: initialData.willing_sell_via_asinyo ?? '',
    willing_repay:         initialData.willing_repay ?? '',
    declaration_date:      initialData.declaration_date ?? '',
  });

  const [interpreterData, setInterpreterData] = useState({
    interpreter_name:      initialData.interpreter_name ?? '',
    interpreter_language:  initialData.interpreter_language ?? '',
    interpreter_address:   initialData.interpreter_address ?? '',
  });

  const [supportNeeded, setSupportNeeded] = useState<string[]>(initialData.support_needed ?? []);
  ...
  const loadOptions = useCallback(async () => {
    ...
    const [{ data: comms }, { data: coops }, { data: progs }, { data: cohs }] = await Promise.all([
      supabase.from('communities').select('id,name,region_code').eq('organisation_id', profile.organisation_id).order('name'),
      supabase.from('cooperatives').select('id,name,community_id').eq('organisation_id', profile.organisation_id).order('name'),
  ...
  // ── Submit ──
  const buildData = (s1Data: S1, s2Data: S2 | null): FarmerFormData => ({
    first_name:    s1Data.first_name,
    last_name:     s1Data.last_name,
    gender:        (s1Data.gender as Gender | '') ?? '',
    date_of_birth: s1Data.date_of_birth ?? '',
    phone:         s1Data.phone,
    region:        (s1Data.region as string | '') ?? '',
    district:      (s1Data.district as string | '') ?? '',
    national_id_type: s2Data?.national_id_type ?? s2?.national_id_type ?? '',
    national_id:      s2Data?.national_id ?? s2?.national_id ?? '',
    photo_url:         photoUrl,
    id_front_url:      idFrontUrl,
    id_back_url:       idBackUrl,
    voice_consent_url: voiceUrl,
    community_id:      s1Data.community_id ?? '',
    cooperative_id:    s1Data.cooperative_id ?? '',
    program_id:        s1Data.program_id ?? '',
    cohort_id_ref:     s1Data.cohort_id_ref ?? '',
    ...farmData,
    ...hhData,
    ...personalData,
    ...occupationData,
  ...
  const handleFinalSubmit = async (andExit = false, s1Override?: S1, s2Override?: S2 | null) => {
    const s1Data = s1Override ?? s1;
    const s2Data = s2Override ?? s2;
    if (!s1Data) return;
    const data = buildData(s1Data, s2Data);
    await onComplete(data, andExit);
  };
  ...
  return (
    <div className={cn('space-y-0', compact ? '' : 'p-4 max-w-lg mx-auto')}>
      ...
      <StepIndicator current={step} onStepClick={mode === 'edit' ? (s) => setStep(s) : undefined} />
      ...
      {step === 0 && (
        <form onSubmit={form1.handleSubmit(d => { setS1(d); setSelectedProgram(d.program_id ?? ''); setStep(1); })} className="space-y-4">
          ...
          <Field label="Region" required error={form1.formState.errors.region?.message}>
            <Select
              value={form1.watch('region') ?? ''}
              onValueChange={v => {
                form1.setValue('region', v);
                form1.setValue('district', '');
                form1.setValue('community_id', '');
                form1.setValue('cooperative_id', '');
              }}
            >
          ...
          <Field label="District">
            <Select
              value={form1.watch('district') ?? ''}
              onValueChange={v => form1.setValue('district', v)}
              disabled={!form1.watch('region')}
            >
              <SelectTrigger>
                <SelectValue placeholder={form1.watch('region') ? 'Select district' : 'Select region first'} />
              </SelectTrigger>
              <SelectContent>
                {(DISTRICTS_BY_REGION[REGION_CODE_BY_NAME[form1.watch('region')] as RegionCode] ?? []).map(d => (
          ...
          <Field label="Community">
            <Select
              ...
              onValueChange={v => {
                form1.setValue('community_id', v === '__none__' ? '' : v);
                form1.setValue('cooperative_id', '');
              }}
              disabled={!form1.watch('region')}
            >
              <SelectTrigger>
                <SelectValue placeholder={form1.watch('region') ? 'Select community' : 'Select region first'} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="__none__">No community</SelectItem>
                {communities
                  .filter(c => {
                    const rcode = REGION_CODE_BY_NAME[form1.watch('region')];
                    return !c.region_code || c.region_code === rcode;
                  })
          ...
          <Field label="Group / Cooperative">
            <Select
              ...
              disabled={!form1.watch('community_id')}
            >
              <SelectTrigger>
                <SelectValue placeholder={form1.watch('community_id') ? 'Select group' : 'Select a community first'} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="__none__">No group</SelectItem>
                {cooperatives
                  .filter(c => c.community_id === form1.watch('community_id') || c.id === form1.watch('cooperative_id'))
                  .map(c => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}
              </SelectContent>
            </Select>
          </Field>
          ...
          <Field label="Residential Address">
            <Input placeholder="House number, street, town" value={personalData.residential_address} onChange={e => setPersonalData(d => ({ ...d, residential_address: e.target.value }))} />
          </Field>
          <YesNoField label="Any Disability?" value={personalData.has_disability} onChange={v => setPersonalData(d => ({ ...d, has_disability: v }))} />
          {personalData.has_disability === 'yes' && (
            <Field label="Specify Disability">
              <Input placeholder="e.g. Visual impairment" value={personalData.disability_specify} onChange={e => setPersonalData(d => ({ ...d, disability_specify: e.target.value }))} />
            </Field>
          )}
          <YesNoField label="Refugee or Displaced Person?" value={personalData.is_refugee_displaced} onChange={v => setPersonalData(d => ({ ...d, is_refugee_displaced: v }))} />
          {personalData.is_refugee_displaced === 'yes' && (
            <Field label="Specify Status">
              <Input placeholder="e.g. Refugee from Burkina Faso" value={personalData.refugee_specify} onChange={e => setPersonalData(d => ({ ...d, refugee_specify: e.target.value }))} />
            </Field>
          )}
          ...
          <div className="flex gap-3">
            {mode === 'edit' && (
              <Button
                ...
                disabled={saving}
                variant="outline"
                className="flex-1 border-cropguard-mid text-cropguard-forest hover:bg-cropguard-mint"
                onClick={() => { const d = form1.getValues() as S1; setS1(d); setSelectedProgram(d.program_id ?? ''); handleFinalSubmit(true, d); }}
              >
                {saving ? <><Loader2 className="w-4 h-4 animate-spin mr-1.5" /> Saving…</> : <><UserCheck className="w-4 h-4 mr-1.5" /> Save & Exit</>}
              </Button>
            )}
          ...
          <div className="flex gap-3">
            ...
            {mode === 'edit' && (
              <Button
                ...
                variant="outline"
                className="flex-1 border-cropguard-mid text-cropguard-forest hover:bg-cropguard-mint"
                onClick={() => { const d = form2.getValues() as S2; setS2(d); handleFinalSubmit(true, undefined, d); }}
              >
                {saving ? <><Loader2 className="w-4 h-4 animate-spin mr-1.5" /> Saving…</> : <><UserCheck className="w-4 h-4 mr-1.5" /> Save & Exit</>}
              </Button>
            )}
      ...
      {step === 2 && (
        <form onSubmit={form3.handleSubmit(() => setStep(3))} className="space-y-4">
          ...
          <SectionTitle>Occupation & Farming</SectionTitle>
          <Field label="Primary Occupation">
            <Select value={occupationData.primary_occupation} onValueChange={v => setOccupationData(d => ({ ...d, primary_occupation: v }))}>
              <SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger>
              <SelectContent>
          ...
          <Field label="Secondary Occupation (optional)">
            <Input placeholder="e.g. Trading" value={occupationData.secondary_occupation} onChange={e => setOccupationData(d => ({ ...d, secondary_occupation: e.target.value }))} />
          </Field>
          <Field label="Type of Farming">
            <Select value={occupationData.farming_type} onValueChange={v => setOccupationData(d => ({ ...d, farming_type: v }))}>
              <SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger>
              <SelectContent>
                {FARMING_TYPE_OPTIONS.map(o => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}
              </SelectContent>
            </Select>
          </Field>
          <Field label="Farm Location">
            <Input placeholder="e.g. Near Asankragua" value={occupationData.farm_location} onChange={e => setOccupationData(d => ({ ...d, farm_location: e.target.value }))} />
          </Field>
          <Field label="Main Buyer / Market">
            <Input placeholder="e.g. Local market, Aggregator" value={occupationData.main_buyer_market} onChange={e => setOccupationData(d => ({ ...d, main_buyer_market: e.target.value }))} />
          </Field>
      ...
      {step === 4 && (
        <div className="space-y-4">
          <SectionTitle>Loan Preferences</SectionTitle>
          <Field label="Loan Type Needed">
            <Select value={financialData.loan_type} onValueChange={v => setFinancialData(d => ({ ...d, loan_type: v }))}>
              <SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger>
              <SelectContent>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Loan Tenor">
              <Select value={financialData.loan_tenor} onValueChange={v => setFinancialData(d => ({ ...d, loan_tenor: v }))}>
                <SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger>
                <SelectContent>
                  {LOAN_TENOR_OPTIONS.map(o => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}
                </SelectContent>
              </Select>
            </Field>
            <Field label="Preferred Repayment">
              <Select value={financialData.preferred_repayment} onValueChange={v => setFinancialData(d => ({ ...d, preferred_repayment: v }))}>
                <SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger>
                <SelectContent>
                  {REPAYMENT_OPTIONS.map(o => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}
                </SelectContent>
              </Select>
            </Field>
          </div>
          <Field label="Average Income (GHS / season)">
            <Input type="number" min="0" placeholder="e.g. 5000" value={financialData.average_income} onChange={e => setFinancialData(d => ({ ...d, average_income: e.target.value }))} />
          </Field>

          <SectionTitle>Bank / Mobile Money</SectionTitle>
          <Field label="Account Type">
            <Select value={financialData.account_type} onValueChange={v => setFinancialData(d => ({ ...d, account_type: v }))}>
              <SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger>
              <SelectContent>
          ...
          {financialData.account_type === 'bank' && (
            <div className="grid grid-cols-2 gap-3">
              <Field label="Bank Name">
                <Input placeholder="e.g. Ecobank" value={financialData.bank_name} onChange={e => setFinancialData(d => ({ ...d, bank_name: e.target.value }))} />
              </Field>
              <Field label="Account Number">
                <Input placeholder="Account number" value={financialData.account_number} onChange={e => setFinancialData(d => ({ ...d, account_number: e.target.value }))} />
              </Field>
            </div>
          )}
          {financialData.account_type === 'mobile_money' && (
            <>
              <Field label="Mobile Money Network">
                <Select value={financialData.mobile_money_network} onValueChange={v => setFinancialData(d => ({ ...d, mobile_money_network: v }))}>
                  <SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger>
                  <SelectContent>
                    {NETWORK_OPTIONS.map(o => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}
                  </SelectContent>
                </Select>
              </Field>
              <Field label="Account Number">
                <Input placeholder="Mobile money number" value={financialData.account_number} onChange={e => setFinancialData(d => ({ ...d, account_number: e.target.value }))} />
              </Field>
            </>
          )}

          <SectionTitle>Financial History</SectionTitle>
          <Field label="Other Income Sources">
            <Input placeholder="e.g. Remittances, Trading" value={financialData.other_income_sources} onChange={e => setFinancialData(d => ({ ...d, other_income_sources: e.target.value }))} />
          </Field>
          <YesNoField label="Any Existing Loans?" value={financialData.existing_loans} onChange={v => setFinancialData(d => ({ ...d, existing_loans: v }))} />
          {financialData.existing_loans === 'yes' && (
            <Field label="Existing Loan Details">
              <Input placeholder="Lender and outstanding amount" value={financialData.existing_loan_detail} onChange={e => setFinancialData(d => ({ ...d, existing_loan_detail: e.target.value }))} />
            </Field>
          )}
          <Field label="Farm Loss History (if any)">
            <textarea rows={2} placeholder="e.g. Lost crops to drought in 2023" value={financialData.farm_loss_history} onChange={e => setFinancialData(d => ({ ...d, farm_loss_history: e.target.value }))} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cropguard-mid resize-none" />
          </Field>
          <YesNoField label="Has Agricultural Insurance?" value={financialData.has_agric_insurance} onChange={v => setFinancialData(d => ({ ...d, has_agric_insurance: v }))} />
          <YesNoField label="Willing to Sell Produce via Asinyo?" value={financialData.willing_sell_via_asinyo} onChange={v => setFinancialData(d => ({ ...d, willing_sell_via_asinyo: v }))} />
          <YesNoField label="Willing to Repay Loan?" value={financialData.willing_repay} onChange={v => setFinancialData(d => ({ ...d, willing_repay: v }))} />
          <Field label="Declaration Date">
            <Input type="date" value={financialData.declaration_date} onChange={e => setFinancialData(d => ({ ...d, declaration_date: e.target.value }))} />
          </Field>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button type="button" className="flex-1 bg-cropguard-dark hover:bg-cropguard-forest" onClick={() => setStep(5)}>
              Next <ChevronRight className="w-4 h-4 ml-1" />
            </Button>
          </div>
        </div>
      ...
      {step === 5 && (
        <div className="space-y-4">
          ...
          <MultiSelect
            label="What support does the farmer need? (select all that apply)"
            options={SUPPORT_NEEDED_OPTIONS}
            value={supportNeeded}
            onChange={setSupportNeeded}
          />

          <SectionTitle>Interpreter (if applicable)</SectionTitle>
          <Field label="Interpreter Name">
            <Input placeholder="e.g. John Doe" value={interpreterData.interpreter_name} onChange={e => setInterpreterData(d => ({ ...d, interpreter_name: e.target.value }))} />
          </Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Language">
              <Input placeholder="e.g. Twi" value={interpreterData.interpreter_language} onChange={e => setInterpreterData(d => ({ ...d, interpreter_language: e.target.value }))} />
            </Field>
            <Field label="Address">
              <Input placeholder="Interpreter's address" value={interpreterData.interpreter_address} onChange={e => setInterpreterData(d => ({ ...d, interpreter_address: e.target.value }))} />
            </Field>
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            {mode === 'edit' && (
              ...
              >
                {saving ? (
                  ...
                  <><UserCheck className="w-4 h-4 mr-1.5" /> Save</>
                )}
              </Button>
            )}
            <Button
            ...
            >
              {saving ? (
                ...
                <><UserCheck className="w-4 h-4 mr-1.5" /> {mode === 'edit' ? 'Save & Exit' : 'Register Farmer'}</>
              )}
            </Button>
          </div>
        </div>

...
export function Main() {
  ...
  const [filterProgram,       setFilterProgram]       = usePersistedState('op-program', '')
  const [filterStatus,        setFilterStatus]        = usePersistedState('op-status', '')
  const [search,              setSearch]              = usePersistedState('op-search', '')
  const [filtersOpen,         setFiltersOpen]         = useState(false)
  const [statsOpen,           setStatsOpen]           = usePersistedState('op-stats', false)
  const [viewMode,            setViewMode]            = usePersistedState<'card' | 'list'>('op-view', 'card')
  ...
  const [pageSize, setPageSize] = usePersistedState('op-page-size', 10)

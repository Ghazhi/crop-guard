...
export default function CreditsRiskScoringPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Filter to enrolled farmers only
      const { data: enrollRows } = await supabase
        .from('enrollments')
        .select('farmer_id')
        .eq('status', 'active')
        .in('farmer_id', ids);
      const enrolledSet = new Set((enrollRows ?? []).map((e: any) => e.farmer_id));
      const enrolledLatest = latest.filter(r => enrolledSet.has(r.farmer_id));
      const enrolledIds = enrolledLatest.map(r => r.farmer_id);
      ...
      let merged: FarmerScore[] = enrolledLatest
        .map(r => {
  ...
  useEffect(() => { load(); }, [load]);
  useEffect(() => { setPage(1); }, [query, zoneFilter, sortKey, sortDesc]);

...
interface EnrollmentWithFarmer {
  ...
  cohort_id: string | null;
  farmers: { id: string; full_name: string; phone: string; region: string; current_fri_score: number | null };
}
...
export default function StaffAgentAssignmentPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (cohortsMapped.length > 0) {
      ...
      const { data: enrollmentAgentMap } = await (supabase.from('enrollments') as any)
        .select('agent_id')
        .in('agent_id', agtList.map(a => a.id))
        .eq('status', 'active');
      const checkinMap: Record<string, number> = {};
      (enrollmentAgentMap ?? []).forEach((e: { agent_id: string | null }) => {
        if (e.agent_id) checkinMap[e.agent_id] = (checkinMap[e.agent_id] ?? 0) + 1;
      });
      ...
      setAgents(agtList.map(a => ({
        id: a.id, full_name: a.full_name, region: a.region_code, is_active: a.is_active,
  ...
  const openAgentFarmers = async (agent: AgentStats) => {
    ...
    const { data: farmers } = await supabase
      .from('farmers')
      .select('id,full_name,phone,region,current_fri_score')
      .in('id', farmerIds)
      .order('full_name');
  ...
  const openFarmerAssign = async (c: CohortWithProgram) => {
    ...
    const { data } = await (supabase.from('enrollments') as any)
      .select('id, farmer_id, agent_id, cohort_id, farmers(id,full_name,phone,region,current_fri_score)')
      .eq('cohort_id', c.id)
      .eq('status', 'active')
    ...
    <div className="p-6 space-y-6">
                      ...
                      <div className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-cropguard-slate mt-0.5">
                            {c.programName} &middot; {c.region ?? '—'} &middot; {c.district}
                          </p>
        ...
        <div className="space-y-4 pt-2">
          {activeCohort && (
            <div className="bg-cropguard-mint rounded-lg px-4 py-3">
              ...
              <p className="text-xs text-cropguard-slate mt-0.5">
                {activeCohort.programName} &middot; {activeCohort.region ?? '—'} &middot; {activeCohort.district}
              </p>
              ...
              {cohorts.map(c => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-cropguard-slate">{c.programName} &middot; {c.region ?? '—'}</p>
                  </div>
                  {c.assignedAgents.length > 0 && (
              ...
              {filteredAgentFarmers.map(f => (
                <div key={f.id} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-cropguard-slate">{f.phone} &middot; {f.region ?? '—'}</p>
      ...
      >
        <div className="flex flex-col h-full gap-4 pt-2">
          ...
          <div className="flex gap-3 mt-auto pt-2 pb-1">

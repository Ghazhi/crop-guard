...
export default function PortfolioAnalyticsPage() {
  ...
  const [uptakePage, setUptakePage] = useState(1);
  const [agentPage, setAgentPage] = useState(1);
  const TABLE_PAGE_SIZE = 10;
  const [uptakeLoadAll, setUptakeLoadAll] = useState(false);
  const [agentLoadAll, setAgentLoadAll] = useState(false);
  ...
  useEffect(() => { setUptakePage(1); setAgentPage(1); }, [progId, cohortId]);

  const uptakePageSize = uptakeLoadAll ? uptake.length : TABLE_PAGE_SIZE;
  const agentPageSize  = agentLoadAll ? agentQual.length : TABLE_PAGE_SIZE;
  const uptakeTotalPages = Math.max(1, Math.ceil(uptake.length / uptakePageSize));
  const agentTotalPages  = Math.max(1, Math.ceil(agentQual.length / agentPageSize));
            ...
            uptake.length === 0
              ...
              : <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {uptake.slice((uptakePage - 1) * uptakePageSize, uptakePage * uptakePageSize).map(u => {
                  ...
                  <Pagination page={uptakePage} totalPages={uptakeTotalPages} onPageChange={setUptakePage} totalItems={uptake.length} pageSize={uptakePageSize} onLoadAll={() => { setUptakeLoadAll(true); setUptakePage(1); }} onResetPaging={() => { setUptakeLoadAll(false); setUptakePage(1); }} />
                </div>
          }
        </CardContent>
      </Card>
            ...
            agentQual.length === 0
              ...
              : <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {agentQual.slice((agentPage - 1) * agentPageSize, agentPage * agentPageSize).map(a => (
                        <tr
                          key={a.agent_id}
                          className={cn(
                  ...
                  <Pagination page={agentPage} totalPages={agentTotalPages} onPageChange={setAgentPage} totalItems={agentQual.length} pageSize={agentPageSize} onLoadAll={() => { setAgentLoadAll(true); setAgentPage(1); }} onResetPaging={() => { setAgentLoadAll(false); setAgentPage(1); }} />
                  <p className="text-xs text-gray-400 px-4 py-2">

...
import {
  type SurveySchedule, type SurveyTemplate,
  SEED_SURVEY_SCHEDULES, SEED_SURVEY_TEMPLATES, PROGRAM_LIST,
} from '../../../_logics/surveyConfig'

function emptySchedule(): SurveySchedule {
  return {
    id: `ss-${Date.now()}`,
...
function ScheduleFormSheet({
  open, schedule, onClose, onSave, surveyTemplates,
}: {
  open: boolean
  ...
  const [draft, setDraft] = useState<SurveySchedule>(schedule ?? emptySchedule())
  ...
  return (
    <SheetTemplate
      open={open}
      onClose={onClose}
      title={schedule ? 'Edit Survey Schedule' : 'New Survey Schedule'}
      size="md"
      footer={
        <>
    ...
    >
      <div className="px-6 py-5 flex flex-col gap-4">
        ...
        <SelectTemplate
          ...
          options={[{ value: '', label: '— None —' }, ...surveyTemplates.map(t => ({ value: t.id, label: t.title }))]}
          value={draft.surveyTemplateId ?? ''}
          onChange={e => setDraft({ ...draft, surveyTemplateId: e.target.value || null })}
        />
      </div>
    </SheetTemplate>
  )
...
export function SurveyScheduleSection() {
  const [schedules, setSchedules] = usePersistedState<SurveySchedule[]>('surveyConfig.schedules', SEED_SURVEY_SCHEDULES)
  const [surveyTemplates] = usePersistedState<SurveyTemplate[]>('surveyConfig.templates', SEED_SURVEY_TEMPLATES)
  ...
  function templateTitle(id: string | null) {
    return id ? surveyTemplates.find(t => t.id === id)?.title ?? 'Unknown template' : null
  }
  ...
  function openEdit(s: SurveySchedule) {
    setEditing(s)
    setFormKey(k => k + 1)
    setSheetOpen(true)
  }
  function handleSave(s: SurveySchedule) {
    setSchedules(prev => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between gap-2">
        <div>
          <h3 className="text-sm font-bold text-gray-900">Survey Schedule</h3>
          <p className="text-xs text-gray-400">Schedule a survey template against a cohort</p>
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-200 p-12 flex flex-col items-center gap-2">
          ...
          <p className="text-sm font-medium text-gray-400 text-center">No survey schedules for this filter.</p>
        </div>
      ) : (
        <div className="flex flex-col gap-3">
          {filtered.map(s => {
            const template = templateTitle(s.surveyTemplateId)
            return (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    {s.isConfigured ? (
                      ...
                      <p className="text-xs text-amber-600">No schedule set — farmers cannot take the survey until configured.</p>
                    )}

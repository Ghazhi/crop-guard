...
import { Search, ChevronRight, Users, CheckCircle, XCircle } from 'lucide-react';
...
export default function CreditsFarmersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      let q = supabase
        .from('farmers')
        .select('id, full_name, phone, region, primary_crop, total_farm_size_ha, gender')
        .order('full_name')
        .limit(150);
      ...
      const [friRes, enrollRes, baselineRes, checkinsRes] = await Promise.all([
        supabase
      ...
      // Only show enrolled farmers
      const enrolledSet = new Set((enrollRes.data ?? []).map((e: any) => e.farmer_id));
      const baselineSet = new Set((baselineRes.data ?? []).map((b: any) => b.farmer_id));

      // Count verified checkins per farmer
      ...
      let rows: FarmerRow[] = (farmerData ?? []).filter((f: any) => enrolledSet.has(f.id)).map((f: any) => ({
        id:            f.id,
        ...
        total_score:   friMap[f.id]?.total_score != null ? Number(friMap[f.id].total_score) : null,
        zone:          friMap[f.id]?.zone ?? null,
        enrolled:      enrolledSet.has(f.id),
  ...
  const pageSize = loadAll ? farmers.length : BASE_PAGE_SIZE;
  const totalPages = Math.max(1, Math.ceil(farmers.length / pageSize));

  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Farmer Profiles</h1>
        <p className="text-sm text-gray-500 mt-1">All farmers with enrollment, baseline, checkin and FRI credit status.</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              ...
              <th className="text-left px-4 py-3 font-semibold text-gray-600 hidden md:table-cell">Crop / Farm</th>
              ...
              <th className="text-left px-4 py-3 font-semibold text-gray-600 hidden lg:table-cell">Baseline</th>
              <th className="text-left px-4 py-3 font-semibold text-gray-600 hidden lg:table-cell">Checkins</th>
              <th className="w-8" />
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {loading
              ...
              : farmers.length === 0
                ...
                : farmers.slice((page - 1) * pageSize, page * pageSize).map(f => (
                    <tr
                      key={f.id}
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 font-semibold text-gray-800">
                        {f.total_score != null ? f.total_score.toFixed(1) : <span className="text-gray-300">—</span>}
                      </td>
                      ...
                      <td className="px-4 py-3.5 hidden lg:table-cell">
                        <span className={cn(
                          ...
                          f.checkins_done > 0 ? 'text-emerald-600' : 'text-gray-400'
                        )}>
                          {f.checkins_done}
                        </span>
                      </td>
        ...
        <Pagination page={page} totalPages={totalPages} onPageChange={setPage} totalItems={farmers.length} pageSize={pageSize} onLoadAll={() => { setLoadAll(true); setPage(1); }} onResetPaging={() => { setLoadAll(false); setPage(1); }} />
      </div>
    </div>
  );
}

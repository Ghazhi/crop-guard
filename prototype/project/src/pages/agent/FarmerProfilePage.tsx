...
import {
  ...
  Brain, Award, Users, ChevronRight,
} from 'lucide-react';
...
import { fetchFarmerClimateRisk, type FarmerClimateRisk } from '@/lib/exposure-data';
import { ClimateRiskPositionCard, ClimateRiskPlaceholder } from '@/components/ClimateRiskPositionCard';
  ...
  return (
    <div className="space-y-3">
      <div className="bg-white rounded-xl border border-gray-100 p-4 space-y-2.5">
        ...
        <Row icon={MapPin}  label="Community"     value={`${farmer.community}, ${farmer.region ?? '—'}`} />
...
interface FRIRowWithCheckin extends FRIRow {
  checkin_id: string | null;
}

function ScoreTab({ farmerId }: { farmerId: string }) {
  const navigate = useNavigate();
  const [rows, setRows] = useState<FRIRowWithCheckin[]>([]);
  const [loading, setLoading] = useState(true);
  const [climateRisk, setClimateRisk] = useState<FarmerClimateRisk | null>(null);
  const [climateLoading, setClimateLoading] = useState(true);

  useEffect(() => {
    supabase.from('farmer_fri_scores')
      .select('week_number, total_score, p1_score, p2_score, p3_score, p4_score, is_provisional, checkin_id')
      .eq('farmer_id', farmerId)
      .order('week_number', { ascending: false })
      .limit(12)
      .then(({ data }) => { setRows((data as FRIRowWithCheckin[]) ?? []); setLoading(false); });
    fetchFarmerClimateRisk(farmerId).then(r => { setClimateRisk(r); setClimateLoading(false); });
  }, [farmerId]);

  if (loading) return <div className="h-40 bg-gray-100 animate-pulse rounded-xl" />;
  if (!rows.length) return (
    <div className="space-y-3">
      <p className="text-sm text-gray-400 text-center py-10">No FRI scores yet.</p>
      {!climateLoading && (climateRisk
        ? <ClimateRiskPositionCard risk={climateRisk} showComponents />
        : <ClimateRiskPlaceholder />)}
    </div>
  );

  const latest = rows[0];

  function handleWeekClick(r: FRIRowWithCheckin) {
    if (r.week_number === 0) {
      navigate(`/agent/farmers/${farmerId}/baseline`);
    } else if (r.checkin_id) {
      navigate(`/agent/verify/${r.checkin_id}`);
    }
  }
  ...
  return (
    <div className="space-y-3">
      <div className="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-4">
        ...
        <div className="flex-1 space-y-2">
          ...
          <p className="text-[10px] text-gray-400">
            {latest.week_number === 0 ? 'Baseline' : `Week ${latest.week_number}`}
          </p>
        </div>
      </div>
      <div className="bg-white rounded-xl border border-gray-100 p-4 space-y-2.5">
      ...
      {!climateLoading && (
        climateRisk ? <ClimateRiskPositionCard risk={climateRisk} showComponents /> : <ClimateRiskPlaceholder />
      )}
      <div className="space-y-1.5">
        {rows.slice(0, 8).map(r => {
          const isClickable = r.week_number === 0 || !!r.checkin_id;
          return (
            <button
              key={r.week_number}
              onClick={() => handleWeekClick(r)}
              disabled={!isClickable}
              className={cn(
                'w-full flex items-center gap-3 bg-white rounded-lg border px-3 py-2.5 text-left transition-colors',
                isClickable
                  ? 'border-gray-100 active:bg-gray-50 cursor-pointer hover:border-cropguard-pale hover:bg-cropguard-mint/30'
            ...
            >
              ...
              <span className="text-xs text-gray-600 flex-1">
                {r.week_number === 0 ? 'Baseline' : `Week ${r.week_number}`}
              </span>
...
function CheckinsTab({ farmerId }: { farmerId: string }) {
  ...
  useEffect(() => {
    supabase.from('farmer_checkins')
      .select('id, week_number, created_at, verified_at, help_requested, challenge_notes, verification_mode, is_agent_only')
      .eq('farmer_id', farmerId)
      .order('week_number', { ascending: false })
      .limit(20)
    ...
    <div className="space-y-2">
      {rows.map(ci => (
        <Link
          ...
          className="block bg-white rounded-xl border border-gray-100 p-3.5 space-y-1.5 active:bg-gray-50 transition-colors"
        >
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-cropguard-forest">Week {ci.week_number}</p>
            <span className={cn('text-[9px] font-bold px-2 py-0.5 rounded-full',
              ci.verified_at
                ? (ci.is_agent_only || ci.verification_mode === 'agent_only'
                  ? 'bg-indigo-100 text-indigo-700'
                  : 'bg-green-100 text-green-700')
                : 'bg-orange-100 text-orange-700')}>
              {ci.verified_at
                ? (ci.is_agent_only || ci.verification_mode === 'agent_only' ? 'Agent Verified' : 'Verified')
                : 'Pending'}
            </span>
          </div>
          ...
          {!ci.verified_at && (
            <span className="block text-center text-[10px] font-bold text-white bg-cropguard-dark rounded-lg py-1.5 mt-1">
              Verify Now
            </span>
          )}
          {ci.verified_at && (
            <span className={cn('block text-center text-[10px] font-bold rounded-lg py-1.5 mt-1',
              ci.is_agent_only || ci.verification_mode === 'agent_only'
                ? 'text-indigo-700 bg-indigo-50'
                : 'text-cropguard-dark bg-cropguard-mint')}>
              View Details
            </span>
          )}
        </Link>
...
export default function FarmerProfilePage() {
  ...
  useEffect(() => {
    if (!farmerId) return;
    supabase.from('farmers').select('*, cooperatives!farmers_cooperative_id_fkey(name)').eq('id', farmerId).maybeSingle()
      .then(({ data, error }) => {
        if (error) { setError(error.message); setLoading(false); return; }
        const f = data as any;
        setFarmer(f as Farmer | null);
        setCooperativeName(f?.cooperatives?.name ?? null);
        setLoading(false);
      });
  }, [farmerId]);
  ...
  if (!farmer) {
    return (
      <div className="p-4 text-center py-16">
        ...
        {error && <p className="text-xs text-red-400 mt-2 max-w-xs mx-auto">{error}</p>}
      ...
      <div className="p-4 pb-0">
        ...
        <div className="flex items-center gap-3 bg-white rounded-2xl border border-gray-100 p-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {cooperativeName && (
              <p className="text-[10px] text-cropguard-mid font-medium flex items-center gap-1 mt-0.5">
                <Users className="w-2.5 h-2.5" /> {cooperativeName}
              </p>
            )}
          </div>

      ...
      <button className="w-full flex items-start justify-between gap-2" onClick={() => setOpen(!open)}>
        <div className="text-left">
          <p className="text-xs font-semibold text-cropguard-forest">
            {report.title ?? TYPE_LABELS[report.report_type as ReportType] ?? report.report_type}
          </p>
...
function ComposeForm({
  ...
  async function handleSubmit() {
    ...
    setSubmitError('');
    try {
      // Resolve a real program_id and cohort name from the agent's active enrollment
      const { data: enrRow } = await supabase
        .from('enrollments')
        .select('program_id, cohort_id, cohorts(name)')
      ...
      const cohortName = (enrRow as any)?.cohorts?.name ?? 'Unassigned';
      const reportTitle = `${TYPE_LABELS[type]}_${cohortName}_${new Date().toISOString().split('T')[0]}_${profile.full_name}`;
...
export default function AgentReportsPage() {
  ...
  const [tab, setTab]               = useState<'submitted' | 'drafts' | 'scores'>('submitted');
  ...
  async function loadSubmitted() {
    ...
    try {
      const { data } = await supabase
        .from('field_reports')
        .select('id, title, report_type, period_start, period_end, content, submitted_at')
        .eq('agent_id', profile.id)
    ...
    <div className="p-4 space-y-4 pb-6">
      ...
      <div className="flex gap-1 bg-gray-100 rounded-xl p-1">
        ...
        <button
          ...
          className={cn('flex-1 text-[10px] font-semibold py-1.5 rounded-lg transition-colors',
            tab === 'scores' ? 'bg-white text-cropguard-dark shadow-sm' : 'text-gray-500')}
        >
          <Award className="w-3 h-3 inline mr-1" />Scores
        </button>
      </div>

      {/* Content */}
      {tab === 'scores' ? (
        <Suspense fallback={<div className="flex items-center justify-center py-20"><Skeleton className="h-16 w-full rounded-xl" /></div>}>
          <FarmerScoresView role="agent" />
        </Suspense>
      ) : loading ? (

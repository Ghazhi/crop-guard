...
const EMPTY_FORM: InterventionForm = {
  ...
  approval_workflow: 'Auto', cohort_id: '', eligibility_rules: [], improvement_steps: [],
};
...
const RULE_FIELDS = [
  ...
  { value: 'exposure_score', label: 'Climate Exposure Score (0–100)' },
  { value: 'quadrant',      label: 'Risk Quadrant (0=HC/LE, 1=HC/HE, 2=LC/LE, 3=LC/HE)' },
];
...
function InterventionDetailModal({
  ...
  const eligibleFarmers = farmers.filter(f => {
    ...
    for (const rule of intervention.eligibility_rules) {
      if (rule.field === 'fri_total') {
        const val = f.fri_score;
        if (rule.operator === '>=' && val < rule.value) return false;
        if (rule.operator === '<=' && val > rule.value) return false;
        if (rule.operator === '==' && val !== rule.value) return false;
        if (rule.operator === '!=' && val === rule.value) return false;
      }
      // exposure_score and quadrant rules are evaluated at read-time from cohort data;
      // the staff-side enrollment drawer doesn't have per-farmer exposure loaded,
      // so these rules are skipped here and enforced by the farmer-facing OppsPage.
    }
    return true;
  });
...
export default function InterventionsPage() {
  ...
  const [cohorts, setCohorts] = useState<{ id: string; name: string; program_id: string }[]>([]);
  ...
  const load = useCallback(async () => {
    if (!profile) return;
    const [{ data: progs }, { data: interventions }, { data: cohortRows }] = await Promise.all([
    ...
    setPrograms(progs ?? []);
    setCohorts((cohortRows ?? []) as { id: string; name: string; program_id: string }[]);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-cropguard-forest">Opportunity Pathways</h1>
      ...
      >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <Select value={form.program_id} onValueChange={v => setForm(f => ({ ...f, program_id: v, cohort_id: '' }))}>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={form.cohort_id || '__none__'} onValueChange={v => setForm(f => ({ ...f, cohort_id: v === '__none__' ? '' : v }))}>
                  <SelectTrigger><SelectValue placeholder="All cohorts" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__none__">All cohorts</SelectItem>
                    {cohorts.filter(c => !form.program_id || c.program_id === form.program_id).map(c => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Opportunity Name *</Label>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex items-start gap-1.5">
                  ...
                  <span title="Actions a farmer can take to improve their FRI score and become eligible for this intervention (e.g. 'Complete 4 consecutive check-ins')" className="text-gray-400 cursor-help text-xs leading-none mt-0.5">(?)</span>
                </div>
          ...
          <div className="flex gap-3 pt-4 border-t mt-4">
            ...
            <Button disabled={saving} className="flex-1 bg-cropguard-dark hover:bg-cropguard-forest" onClick={handleSave}>
              {saving ? <><Loader2 className="w-4 h-4 animate-spin mr-2" />Saving…</> : editing ? 'Save Changes' : 'Create Opportunity'}
            </Button>
          </div>
      </Drawer>

      {/* Delete confirm Drawer */}
      <Drawer open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)} title="Delete Opportunity">
          <div className="space-y-4">

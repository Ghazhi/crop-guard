...
export default function CreditsPortfolioPage() {
  ...
  const pageSize = loadAll ? cohortSummary.length : BASE_PAGE_SIZE;
  const cohortTotalPages = Math.max(1, Math.ceil(cohortSummary.length / pageSize));
      ...
      {!loading && cohortSummary.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
          ...
          <div className="divide-y divide-gray-50">
            {cohortSummary.slice((page - 1) * pageSize, page * pageSize).map(c => (
          ...
          <Pagination page={page} totalPages={cohortTotalPages} onPageChange={setPage} totalItems={cohortSummary.length} pageSize={pageSize} onLoadAll={() => { setLoadAll(true); setPage(1); }} onResetPaging={() => { setLoadAll(false); setPage(1); }} />
        </div>
      )}
    </div>
  );

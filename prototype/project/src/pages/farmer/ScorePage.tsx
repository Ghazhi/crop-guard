...
export default function ScorePage() {
  ...
  useEffect(() => {
    ...
    supabase.from('farmers').select('id').eq('user_id', profile.id).maybeSingle()
      .then(({ data: f }) => {
        if (!f) { setLoading(false); setClimateLoading(false); return; }
        ...
        fetchFarmerClimateRisk(f.id).then(r => { setClimateRisk(r); setClimateLoading(false); });
      });
  }, [profile]);
  ...
  return (
    <div className="p-4 space-y-4 pb-6">
          ...
          <div className="flex-1 space-y-3">
            ...
            <div className="text-xs text-cropguard-slate leading-relaxed">
              {currentScore === null
                ...
                : `${latest?.week_number === 0 ? 'Baseline assessment' : `Week ${latest?.week_number} result`}. ${latest.is_provisional ? 'Agent verification pending.' : 'Verified by agent.'}`}
            </div>
          </div>
        </div>
      </div>
      ...
      {!climateLoading && (
        climateRisk ? <ClimateRiskPositionCard risk={climateRisk} /> : <ClimateRiskPlaceholder />
      )}

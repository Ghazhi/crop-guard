...
// ── PM scope ──────────────────────────────────────────────────────────────────
// Only surface partners / programs / interventions / farmers linked to the
// signed-in Program Manager's programs.

const PM_PROGRAMS = PROGRAMS.filter(prog => PM_PROGRAM_IDS.includes(prog.id))

const PM_FARMERS = FARMERS_LIST.filter(f => isPmProgram(f.enrollment?.programId))

const INITIAL_PARTNERS: Partner[] = PARTNERS
  .filter(p => PM_PARTNER_IDS.includes(p.id))
  .map(p => ({
    ...p,
    programs: PM_PROGRAMS.filter(prog => prog.partnerId === p.id).length,
  }))
...
// ── View Partner Sheet ────────────────────────────────────────────────────────
// Linked programs / interventions / farmers are derived from the data center,
// scoped to the PM's programs for the selected partner.

interface LinkedCohort {
  id: string; name: string; enrolled: number; target: number; compliance: number
}

interface LinkedProgram {
  id: string; name: string; season: string; cohorts: LinkedCohort[]
  totalFarmers: number; status: string
}

interface LinkedIntervention {
  id: string; name: string; type: string; beneficiaries: number
  disbursed: string; repaymentRate: number | null; status: string
}

interface ScopedFarmer {
  id: string; name: string; community: string; friScore: number
  programId: string; cohortId: string | null
}

const SCOPED_FARMER_COLUMNS: DatagridColumn<ScopedFarmer>[] = [
...
function cohortCompliance(cohortId: string): number {
  const members = PM_FARMERS.filter(f => f.enrollment?.cohortId === cohortId)
  if (members.length === 0) return 0
  return Math.round((members.filter(f => f.currentFri !== null).length / members.length) * 100)
}

function linkedProgramsFor(partnerId: string): LinkedProgram[] {
  return PM_PROGRAMS
    .filter(prog => prog.partnerId === partnerId)
    .map(prog => ({
      id:     prog.id,
      name:   prog.name,
      season: prog.season,
      cohorts: prog.cohorts.map(c => ({
        id:         c.id,
        name:       c.name,
        enrolled:   c.enrolledCount,
        target:     c.targetCount,
        compliance: cohortCompliance(c.id),
      })),
      totalFarmers: prog.enrolledCount,
      status:       prog.status,
    }))
}

function linkedInterventionsFor(partnerId: string): LinkedIntervention[] {
  return INTERVENTIONS
    .map(i => {
      const assignment = (i.partnerAssignments ?? []).find(a => a.partnerId === partnerId)
      const pmCohorts  = (assignment?.cohorts ?? []).filter(c => isPmProgram(c.programId))
      return { i, pmCohorts }
    })
    .filter(({ pmCohorts }) => pmCohorts.length > 0)
    .map(({ i, pmCohorts }) => ({
      id:   i.id,
      name: i.name,
      type: i.type,
      beneficiaries: PM_FARMERS.filter(f => pmCohorts.some(c => c.cohortId === f.enrollment?.cohortId)).length,
      disbursed:     i.valueDescription,
      repaymentRate: null,
      status:        i.status,
    }))
}

function farmersFor(partnerId: string): ScopedFarmer[] {
  const programIds = PM_PROGRAMS.filter(prog => prog.partnerId === partnerId).map(prog => prog.id)
  return PM_FARMERS
    .filter(f => f.enrollment && programIds.includes(f.enrollment.programId))
    .map(f => ({
      id:        f.id,
      name:      f.fullName,
      community: f.community,
      friScore:  f.currentFri ?? 0,
      programId: f.enrollment!.programId,
      cohortId:  f.enrollment!.cohortId,
    }))
}
...
function ViewPartnerSheet({ partner, onClose, onRemove, onEdit }: {
  ...
  // PM-scoped linked data for this partner
  const linkedPrograms      = linkedProgramsFor(p.id)
  const linkedInterventions = linkedInterventionsFor(p.id)
  const partnerFarmers      = farmersFor(p.id)

  // Overview quick stats
  const totalFarmers = linkedPrograms.reduce((sum, prog) => sum + prog.totalFarmers, 0)
  const activePrograms = linkedPrograms.filter(prog => prog.status === 'Active').length
  const repaymentRates = linkedInterventions.map(i => i.repaymentRate).filter((r): r is number => r !== null)
  ...
  const filteredPrograms = linkedPrograms
  ...
  const cohortOptions = programFilter
    ? (linkedPrograms.find(prog => prog.name === programFilter)?.cohorts ?? [])
    : linkedPrograms.flatMap(prog => prog.cohorts)
            ...
            <div className="pt-3 space-y-3">
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  {linkedPrograms.map(prog => <option key={prog.id}>{prog.name}</option>)}
                </select>
                <select
                  value={cohortFilter}
                ...
                return (
                  <CardTemplate key={prog.id} noPadding className="overflow-hidden">
                    <div className="px-6 pt-4 pb-3">
                      ...
                      <p className="text-xs text-gray-400 mb-3">{prog.season}</p>
          ...
          {tab === 'interventions' && (
            <div className="pt-3 space-y-3">
              ...
              {linkedInterventions.length === 0 && (
                <div className="text-center py-10 text-gray-400 text-sm">No interventions linked to this partner</div>
              )}
            </div>
          )}
        </div>
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[80vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
              ...
              {!farmersModal.fromIntervention && (
                <div className="flex gap-2 px-5 py-3 border-b border-gray-100">
                  ...
                  >
                    <option value="">All Programs</option>
                    {linkedPrograms.map(prog => <option key={prog.id} value={prog.id}>{prog.name}</option>)}
                  </select>
                  <select
                    value={fmCohort}
                  ...
                  >
                    <option value="">All Cohorts</option>
                    {(fmProgram
                      ? linkedPrograms.find(prog => prog.id === fmProgram)?.cohorts ?? []
                      : linkedPrograms.flatMap(prog => prog.cohorts)
              ...
              <div className="flex-1 overflow-y-auto px-5 py-3">
                <DatagridTemplate<ScopedFarmer>
                  columns={SCOPED_FARMER_COLUMNS}
                  data={partnerFarmers.filter(f =>
...
export function Main() {
  ...
  // Overview stats (recomputed from the PM-scoped partner list)
  ...
  const PARTNER_COLUMNS: DatagridColumn<Partner>[] = [
    {
      ...
      render: (v, p) => (
        <button onClick={() => setViewPartner(p)} className="flex items-center gap-3 group text-left">
          <PersonAvatar name={p.name} size={32} shape="square" />
          <span className="font-medium text-gray-900 group-hover:text-green-700 transition-colors">{String(v)}</span>
        </button>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, p) => (
        <div className="flex items-center justify-end gap-1">
          ...
          <button onClick={() => { setEditPartner(p); setEditOpen(true) }}
            className="flex items-center gap-1.5 h-7 px-2.5 rounded-lg text-xs font-medium text-gray-600 hover:bg-gray-100 transition-colors">
            <Pencil className="w-3.5 h-3.5" /> Edit
          </button>

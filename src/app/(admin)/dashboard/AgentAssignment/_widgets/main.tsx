...
import { Search, Users, AlertTriangle, UserCog, X, UserRoundPlus, UserPlus, SlidersHorizontal, ChevronDown, BarChart2, ChevronUp } from 'lucide-react'
...
function BulkAssignSheet({ open, onClose, agents, cohorts }: {
  ...
  function toggleCohort(id: string) {
    setCohortIds(prev => { const n = new Set(prev); if (n.has(id)) { n.delete(id) } else { n.add(id) } return n })
  }
...
export function Main() {
  ...
  const [filterAgent,   setFilterAgent]   = usePersistedState('aa-agent', '')
  const [filterProgram, setFilterProgram] = usePersistedState('aa-program', '')
  const [filterCohort,  setFilterCohort]  = usePersistedState('aa-cohort', '')
  const [search,        setSearch]        = usePersistedState('aa-search', '')
  const [filtersOpen,   setFiltersOpen]   = useState(false)
  const [statsOpen,     setStatsOpen]     = usePersistedState('aa-stats', false)
  ...
  const [pageSize, setPageSize] = usePersistedState('aa-page-size', 10)
  // eslint-disable-next-line react-hooks/set-state-in-effect
  ...
  return (
    <div className="p-6 space-y-6" style={{ background: 'var(--brand-gray)', minHeight: '100vh' }}>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-3">
            <div className="flex items-center gap-2 flex-wrap">
              <div className="relative flex-1 min-w-48">
                ...
                <input
                  className="w-full border border-gray-200 rounded-lg pl-9 pr-9 h-10 text-sm focus:outline-none focus:ring-2 focus:ring-(--brand-dark)/20 focus:border-(--brand-dark) transition-colors bg-white"
              ...
              <div className="flex items-center gap-2 flex-wrap pt-1 border-t border-gray-50">
                {filterProgram && (
                  <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full" style={{ background: 'var(--brand-pale)', color: 'var(--brand-dark)' }}>
                    ...
                    <X className="w-3 h-3 cursor-pointer" onClick={() => { setFilterProgram(''); setFilterCohort('') }} />
                  </span>
                )}
                {filterCohort && (
                ...
                <button onClick={() => { setFilterProgram(''); setFilterCohort('') }} className="text-xs text-gray-400 hover:text-gray-600 flex items-center gap-1 ml-auto">
      ...
      {!loading && displayed.length > 0 && (
        <PaginationBar
          page={page} pageSize={pageSize} total={displayed.length}
                ...
                <div key={c.cohortId}>
                  ...
                  <div className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50/60 transition-colors">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <ButtonTemplate variant="outline" size="sm" isIcon tooltip="Farmers"
                      ...
                      {isUnassigned ? (
                        ...
                        <ButtonTemplate variant="outline" size="sm" isIcon tooltip="Add Agent"

...
function evalRule(facts: FarmerFacts, field: string, operator: string, value: number): boolean {
  const map: Record<string, number | null> = {
    ...
    exposure_score:  facts.exposure_score,
    quadrant:        facts.quadrant,
  };
  const v = map[field] ?? 0;
...
export default function OppsPage() {
  ...
  const load = useCallback(async () => {
    ...
    // 3b. Climate exposure — resolve farmer's cohort, then fetch exposure inputs
    let exposureScore = 0;
    let quadrantVal = 0;
    if (enrRow?.id) {
      const { data: enrFull } = await supabase
        ...
        .eq('id', enrRow.id)
        .maybeSingle();
      const cohortId = (enrFull as any)?.cohort_id;
      if (cohortId) {
        const { data: expRow } = await (supabase.from('cohort_exposure_inputs') as any)
          .select('*')
          .eq('cohort_id', cohortId)
          .maybeSingle();
        if (expRow) {
          const { computeExposureScore, computeQuadrant } = await import('@/lib/exposure');
          exposureScore = computeExposureScore({
            hazard_classification: expRow.hazard_classification,
            actual_rainfall: Number(expRow.actual_rainfall) || 0,
            historical_avg_rainfall: Number(expRow.historical_avg_rainfall) || 0,
            critical_alert_count: expRow.critical_alert_count || 0,
            high_alert_count: expRow.high_alert_count || 0,
            medium_alert_count: expRow.medium_alert_count || 0,
            in_critical_growth_stage: expRow.in_critical_growth_stage ?? false,
            forecast_stress_flag: expRow.forecast_stress_flag ?? false,
          }).score;
          const fri = scoreRow?.total_score ?? 0;
          const qk = computeQuadrant(fri, exposureScore);
          quadrantVal = qk === 'HighCap_LowExp' ? 0 : qk === 'HighCap_HighExp' ? 1 : qk === 'LowCap_LowExp' ? 2 : 3;
        }
      }
    }

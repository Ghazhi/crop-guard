...
const totalFarmers     = FARMERS_LIST.length
const verifiedFarmers  = FARMERS_LIST.filter(f => f.currentFri !== null).length
const verifiedPct      = Math.round((verifiedFarmers / totalFarmers) * 100)
const activeEnroll     = DASHBOARD_STATS.activeEnrollments
const friScores        = FARMERS_LIST.map(f => f.currentFri).filter((s): s is number => s !== null)
const avgFri           = friScores.length ? Math.round(friScores.reduce((a, b) => a + b, 0) / friScores.length) : 0
const openRiskFlags    = 0
const checkinsLast7    = 1

const ENROLL_DATA = PROGRAMS.map(p => ({ name: p.name, enrolled: p.enrolledCount }))
...
const LEADERBOARD_DATA = AGENTS.map(a => ({
  name: a.name.split(' ')[0],
...
function generateReport(typeId: string, programFilter: string): { profile: string; recommendation: string } | null {
  const prog = programFilter === 'all' ? 'All Programs' : PROGRAMS.find(p => p.id === programFilter)?.name ?? 'All Programs'
  switch (typeId) {
    case 'portfolio':
      return {
        profile:        `${totalFarmers} farmers registered across ${PROGRAMS.length} active programs. ${verifiedFarmers} farmers have completed FRI assessments (${verifiedPct}% verification rate). Average FRI score is ${avgFri}. Zone breakdown: ${ZONE_BREAKDOWN.map(z => `${z.count} ${z.zone}`).join(', ')}.`,
    ...
    case 'agent':
      return {
        profile:        `${AGENTS.length} agents active across ${[...new Set(AGENTS.flatMap(a => a.regions))].length} regions. Kwame Asante: ${AGENTS[0].checkinCount} check-ins, ${AGENTS[0].farmerCount} farmers. Abdul Razak: ${AGENTS[1].checkinCount} check-ins, ${AGENTS[1].farmerCount} farmers. Overall check-in rate is below target.`,
        recommendation: `Abdul Razak has 0 check-ins recorded. Follow up to confirm check-in data is being submitted. Set a weekly check-in target of at least 2 per assigned farmer.`,
      }
    case 'intervention':
      return {
        profile:        `4 interventions active across ${PROGRAMS.length} programs in ${prog}. ${activeEnroll} farmers currently enrolled. Intervention uptake rate is below programme capacity — most opportunities have significant remaining slots.`,
    ...
    case 'risk':
      return {
        profile:        `${openRiskFlags} farmers are in high or critical FRI zones. 1 help request was flagged by farmers during check-ins. ${FARMERS_LIST.filter(f => f.currentFri !== null && f.currentFri < 50).length} farmers are in the medium risk zone and require monitoring.`,

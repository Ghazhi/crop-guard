...
import {
  ...
  Target, Zap, FileText, Award, Landmark,
} from 'lucide-react';
import { cn } from '@/lib/utils';
import { useAuthStore } from '@/store/auth';

const NAV_ITEMS = [
  { to: '/partner/norvi',           icon: Brain,           label: 'Intelligence Dashboard' },
  { to: '/partner/merl',            icon: Target,          label: 'MERL Dashboard'          },
  { to: '/partner/interventions',   icon: Zap,             label: 'Interventions'           },
  { to: '/partner/cohorts',          icon: Award,           label: 'Cohort Performance'     },
  { to: '/partner/cooperatives',    icon: Landmark,        label: 'Cooperative Governance' },
  { to: '/partner/reports',          icon: FileText,        label: 'Reports & Exports'       },
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className={cn(
        ...
        <div className={cn(
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              ...
              <p className="text-cropguard-pale text-xs mt-0.5">Partner / MERL</p>
            </div>
          )}
        ...
        <div className="border-t border-white/10 p-3">
          {!collapsed && (
            <button
              onClick={() => navigate('/partner/configuration?tab=profile')}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-white text-sm font-medium truncate group-hover:text-cropguard-light transition-colors">{profile?.full_name || 'Partner'}</p>

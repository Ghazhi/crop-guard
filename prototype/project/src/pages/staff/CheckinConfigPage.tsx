  ...
  return (
    <div className="space-y-3">
          ...
          return (
            <button key={comp} type="button" onClick={() => setValues(v => ({ ...v, component: comp }))}
              className={cn('text-xs px-2.5 py-1 rounded-lg border font-medium transition-colors',
                values.component === comp ? `${meta.bg} ${meta.border} ${meta.color} border-2` : 'bg-white border-gray-200 text-gray-500 hover:border-gray-300')}>
              {meta.label}
            </button>
          );
        })}
      ...
      <div className="flex gap-2">
        <Button onClick={() => onSave(values)} disabled={saving || !values.label.trim()} size="sm" className="bg-cropguard-forest text-white hover:bg-cropguard-dark">
...
function WeeklyQuestionsSection() {
  ...
  const [cropTab, setCropTab] = useState<CropType>('soybean');
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .in('crop_type', ['maize', 'soybean', 'cocoa'])
      .order('week_number')
      .order('sort_order');
  ...
  const handleEdit = async (id: string, values: { component: CheckinComponent; label: string; description: string; is_active: boolean }) => {
    setSaving(true);
    const { error } = await supabase.from('checkin_questions').update({ ...values, label: values.label.trim(), description: values.description.trim() }).eq('id', id);
    setSaving(false);
    if (error) { addToast('Failed to update', 'error'); return; }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-1 bg-gray-100 rounded-xl p-1 w-fit">
        {(['maize', 'soybean', 'cocoa'] as CropType[]).map(crop => (
        ...
        <div className="space-y-3">
          ...
          {!hasQuestions ? (
            <div className="flex items-center justify-between gap-4 p-4 bg-amber-50 border border-amber-200 rounded-xl">
              ...
              <Button onClick={seedFromTemplates} disabled={seeding} size="sm" className="shrink-0 bg-cropguard-forest text-white hover:bg-cropguard-dark gap-1.5">
                                    ...
                                    {editingId === q.id ? (
                                      ...
                                      <div className="flex items-start gap-2 px-4 py-3 bg-white hover:bg-gray-50 transition-colors">
                                        <div className="flex-1 min-w-0">
                                          <p className={cn('text-xs font-medium leading-snug', q.is_active ? 'text-cropguard-forest' : 'text-gray-400 line-through')}>
                                            {q.label}
                                          </p>
                                        ...
                                        <div className="flex items-center gap-2 shrink-0">
                                          <button type="button" onClick={() => toggleActive(q)}
                                            ...
                                            className={cn('relative h-5 w-9 rounded-full transition-colors', q.is_active ? 'bg-emerald-500' : 'bg-gray-300')}>
                                            <div className={cn('absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform', q.is_active ? 'translate-x-[18px]' : 'translate-x-0.5')} />
                                          </button>
                                          <button type="button" onClick={() => { setEditingId(q.id); setCreatingWeek(null); }}
...
function CohortSchedulesSection() {
  ...
  const saveEdit = async () => {
    ...
    setCohorts(prev => prev.map(c => c.id === editing.id ? { ...c, checkin_start_date: editing.start_date, checkin_window_days: editing.window_days, checkin_grace_days: editing.grace_days } : c));
                ...
                {!isEditing ? (
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <Input type="date" value={editing.start_date} onChange={e => setEditing({ ...editing, start_date: e.target.value })} className="text-sm" />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs font-medium text-gray-600 mb-1">Window (days)</label>
                        <Input type="number" min="1" max="14" value={editing.window_days} onChange={e => setEditing({ ...editing, window_days: parseInt(e.target.value) || 7 })} className="text-sm" />
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-gray-600 mb-1">Grace period (days)</label>
                        <Input type="number" min="0" max="7" value={editing.grace_days} onChange={e => setEditing({ ...editing, grace_days: parseInt(e.target.value) || 0 })} className="text-sm" />
                      </div>
                    </div>
                    <div className="flex gap-2 pt-1">
                      <Button onClick={saveEdit} disabled={savingId !== null} size="sm" className="bg-cropguard-forest text-white hover:bg-cropguard-dark">
                    ...
                    <div key={activity.id} className="flex items-start gap-4 p-4 bg-white hover:bg-gray-50 transition-colors">
                      ...
                      <button type="button" onClick={() => toggleActivity(activity.id, activity.is_active)} disabled={!useDb}
                        className={cn('relative h-6 w-11 rounded-full transition-colors shrink-0 mt-0.5',
                          activity.is_active ? 'bg-emerald-500' : 'bg-gray-300', !useDb && 'opacity-50 cursor-not-allowed')}>

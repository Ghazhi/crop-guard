...
import {
  ...
  TrendingUp, Settings2, UserCog, Landmark, Award,
} from 'lucide-react';
...
const NAV_ITEMS = [
  ...
  { to: '/agronomist/fri',           icon: TrendingUp,      label: 'FRI Dashboard'   },
  { to: '/agronomist/scores',        icon: Award,           label: 'Farmer Scores'   },
  { to: '/agronomist/checkin-config',icon: Settings2,       label: 'Check-in Config' },
  { to: '/agronomist/agents',        icon: UserCog,         label: 'Agent Management'},
  { to: '/agronomist/interventions', icon: Zap,             label: 'Trainings'       },
  { to: '/agronomist/advisory',      icon: BookOpen,        label: 'Advisory Notes'  },
  { to: '/agronomist/cooperatives',  icon: Landmark,        label: 'Cooperative Governance' },
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className={cn(
        ...
        <div className="border-t border-white/10 p-3">
          {!collapsed && (
            <button
              onClick={() => navigate('/agronomist/configuration?tab=profile')}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-white text-sm font-medium truncate group-hover:text-emerald-200 transition-colors">{profile?.full_name || 'Agronomist'}</p>
      ...
      <div className={cn('flex-1 flex flex-col min-h-screen transition-all duration-200', mainML)}>
        <header className="sticky top-0 z-10 bg-white border-b border-gray-200 h-14 flex items-center px-4 gap-3">
          ...
          >
            ...
            <span className="text-sm text-gray-700 hidden sm:block group-hover:text-emerald-700 transition-colors">{profile?.full_name}</span>
          </button>
        </header>

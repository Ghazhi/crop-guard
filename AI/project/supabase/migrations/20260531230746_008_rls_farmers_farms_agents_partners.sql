...
CREATE POLICY "Agents view farmers in their org"
  ON farmers FOR SELECT TO authenticated
  USING ((get_my_role() = 'agent'::user_role) AND (organisation_id = get_my_org_id()));
...
CREATE POLICY "Agents can register new farmers"
  ...
  WITH CHECK (is_agent_or_above() AND organisation_id = get_my_org_id());

CREATE POLICY "Agents can update farmers they enrolled"
  ON farmers FOR UPDATE TO authenticated
  USING (is_agent_or_above() AND organisation_id = get_my_org_id())
  WITH CHECK (is_agent_or_above() AND organisation_id = get_my_org_id());
...
CREATE POLICY "Agents view farm details of their farmers"
  ON farm_details FOR SELECT TO authenticated
  USING (get_my_role() = 'agent' AND EXISTS (SELECT 1 FROM enrollments WHERE enrollments.farmer_id = farm_details.farmer_id AND enrollments.agent_id = auth.uid()));

CREATE POLICY "Staff and admin view farm details in their org"
  ON farm_details FOR SELECT TO authenticated
  USING (is_admin_or_staff() AND EXISTS (SELECT 1 FROM farmers WHERE farmers.id = farm_details.farmer_id AND farmers.organisation_id = get_my_org_id()));

CREATE POLICY "Agents can insert farm details"
  ON farm_details FOR INSERT TO authenticated
...
CREATE POLICY "Agents can update farm details"
  ON farm_details FOR UPDATE TO authenticated
  USING (is_agent_or_above() AND EXISTS (SELECT 1 FROM farmers WHERE farmers.id = farm_details.farmer_id AND farmers.organisation_id = get_my_org_id()))
  WITH CHECK (is_agent_or_above() AND EXISTS (SELECT 1 FROM farmers WHERE farmers.id = farmer_id AND farmers.organisation_id = get_my_org_id()));

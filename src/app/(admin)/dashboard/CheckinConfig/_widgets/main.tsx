...
type Section = 'weekly' | 'cohort' | 'baseline' | 'crops'

interface CropDef { id: string; name: string; builtIn?: boolean }
...
// ─── built-in crop definitions ───────────────────────────────────────────────

const BUILT_IN_CROPS: CropDef[] = [
...
const BUILT_IN_SEED_MAP: Record<string, Week[]> = {
  maize: MAIZE_SEED, soybean: SOYBEAN_SEED, rice: RICE_SEED,
  cassava: CASSAVA_SEED, yam: YAM_SEED, groundnut: GROUNDNUT_SEED,
  cowpea: COWPEA_SEED, tomato: TOMATO_SEED,
}
...
function freshConfig(cropDefs: CropDef[]): OrgConfig {
  return {
...
export function Main() {
  ...
  // ── crop tab display: first 4 visible, rest in overflow multi-select ────────
  const [pinnedOverflow, setPinnedOverflow] = useState<string[]>([])
  const [overflowOpen,   setOverflowOpen]   = useState(false)

  const VISIBLE_TABS  = 4
  const tabCrops      = crops.slice(0, VISIBLE_TABS)
  const overflowCrops = crops.slice(VISIBLE_TABS)
  // pinned = overflow crops the user checked → shown as extra tabs
  const pinnedTabs    = overflowCrops.filter(c => pinnedOverflow.includes(c.id))

  function togglePinned(id: string) {
    setPinnedOverflow(prev =>
        ...
        <div className="flex-1 min-w-0">
          ...
          {section === 'weekly' && (
            <div className="flex flex-col gap-4">
              ...
              {/* crop tabs: first 4 + pinned overflow + multi-select dropdown */}
              <div className="flex items-center gap-1 flex-wrap">
                <div className="flex items-center gap-1 bg-gray-100 rounded-xl p-1 flex-wrap">
                  {tabCrops.map(c => (
                    <button key={c.id} onClick={() => setCrop(c.id)}
                      className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all whitespace-nowrap ${
                        crop === c.id ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'
                      }`}>
                      {c.name}
                    </button>
                  ))}
                  {pinnedTabs.map(c => (
                    <span key={c.id}
                      ...
                      <button onClick={() => setCrop(c.id)}>{c.name}</button>
                      <button onClick={() => { togglePinned(c.id); if (crop === c.id) setCrop(tabCrops[0]?.id ?? '') }}
                ...
                {overflowCrops.length > 0 && (
                  <div className="relative">
                    ...
                    >
                      More
                      {pinnedOverflow.length > 0 && (
                        <span className="flex items-center justify-center w-4 h-4 rounded-full text-[10px] font-bold text-white" style={{ background: 'var(--brand-forest)' }}>
                          {pinnedOverflow.length}
                        </span>
                      )}
                      <ChevronDown className={`w-3.5 h-3.5 transition-transform ${overflowOpen ? 'rotate-180' : ''}`} />
                    </button>
                    ...
                    {overflowOpen && (
                      <>
                        ...
                        <div className="absolute left-0 top-full mt-1 z-20 bg-white rounded-xl border border-gray-200 shadow-lg py-1 min-w-40">
                          {overflowCrops.map(c => {
                            const checked = pinnedOverflow.includes(c.id)
                            return (
                              <button key={c.id}
                                onClick={() => { togglePinned(c.id); if (!checked) setCrop(c.id) }}
                                ...
                                <span className={checked ? 'text-gray-900 font-medium' : 'text-gray-600'}>{c.name}</span>
                              </button>
                            )
                          })}
                        </div>

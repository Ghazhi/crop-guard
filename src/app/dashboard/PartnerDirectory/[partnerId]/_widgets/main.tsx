...
import type { Intervention, EnrolledCohort } from '@/app/dashboard/OpportunityPathways/_logics/interface'
...
function OpportunityDetailModal({ intervention, cohorts, onClose }: {
  ...
  const programOptions = useMemo(() => {
    ...
    cohorts.forEach(ec => { if (!seen.has(ec.programId)) seen.set(ec.programId, ec.programName) })
  ...
  const cohortOptions = useMemo(() => {
    const base = filterProgram ? cohorts.filter(ec => ec.programId === filterProgram) : cohorts
    const seen = new Map<string, string>()
    base.forEach(ec => { if (!seen.has(ec.cohortId)) seen.set(ec.cohortId, ec.cohortName) })
    return Array.from(seen.entries()).map(([id, name]) => ({ id, name }))
...
function AssignSheet({ open, onClose, assignedIds, onAssign }: {
  ...
  function handleNext() {
    ...
    setSelectedCohortKeys(new Set(selectedOpp.enrolledCohorts.map(ec => `${ec.programId}::${ec.cohortId}`)))
    setStep('pick-cohorts')
  }

  function toggleCohort(key: string) {
    setSelectedCohortKeys(prev => { const n = new Set(prev); n.has(key) ? n.delete(key) : n.add(key); return n })
  }

  function handleSave() {
    if (!selectedOpp) return
    const cohorts = selectedOpp.enrolledCohorts.filter(ec => selectedCohortKeys.has(`${ec.programId}::${ec.cohortId}`))
  ...
  const cohortsByProgram = useMemo(() => {
    ...
    selectedOpp.enrolledCohorts.forEach(ec => {
...
function LinkedProgramsTab({ opportunities }: { opportunities: { intervention: Intervention; cohorts: EnrolledCohort[] }[] }) {
  ...
  const rows = useMemo<ProgRow[]>(() => {
    ...
    return Array.from(seen.entries()).map(([programId, { cohortIds }]) => {
      ...
      const ec0 = opportunities.flatMap(o => o.cohorts).find(ec => ec.programId === programId)
...
function LinkedInterventionsTab({
  opportunities, assignedIds, onRemove, onAssign, onDetail,
}: {
...
export function Main({ partnerId }: { partnerId: string }) {
  ...
  const [assignments, setAssignments] = useState<Assignment[]>(() =>
    INTERVENTIONS.flatMap(i => {
      const pa = i.partnerAssignments?.find(a => a.partnerId === partnerId)
  ...
  const uniqueCohorts = useMemo(() =>
    new Set(opportunities.flatMap(({ cohorts }) => cohorts.map(ec => ec.cohortId))).size,
  [opportunities])
